...
The default new test is the highest practical public boundary: an Express route, service workflow, persisted-store adapter, socket exchange, or rendered user interaction. Cover the success path plus the materially different failure/compatibility paths; do not enumerate every internal branch because it exists.

Do not add or keep a unit test that only raises coverage for a deterministic helper already exercised through a stable caller. When a helper's signature and every caller would change together, prefer the caller's integration contract and delete redundant helper-level examples. Table-driven permutations that all prove the same product outcome are duplication.

Focused unit tests remain valuable where a higher-level test cannot pin behavior precisely or cheaply: parsers and algorithms with a real input matrix; security, privacy, data-isolation, and destructive-action guards; migrations and cross-version compatibility; serialization/schema boundaries; retry, timeout, and process-lifecycle state machines; pure edge cases whose failure would be ambiguous through an integration test. Real timeout behavior gets one contract test with injected/fake time — never repeated production sleeps.

Before adding a test, name the regression it uniquely catches. In review, remove tests that duplicate a stronger boundary assertion, mirror implementation details, only verify mocks called other mocks, or cover impossible states. Test count and line coverage are diagnostics; CI time, determinism, and regression-detection value are the goals.
...
Because both are off by default, **never treat "the password is set" as an available guarantee** — gate on it explicitly, or design for the default posture. **PII must not ride the federation layer at all** — ADR [privacy records machine-local](docs/decisions/2026-08-08-privacy-records-machine-local.md). That governs *records* and *status/capability payloads*. The only carve-outs are a submitted job body carrying the conditioning an allowlisted peer is asked to render — the prompt (ADR [federated visual prompts](docs/decisions/2026-08-20-federated-visual-prompts.md)) and, under the same gate, lyrics and conditioning images (ADR [conditioning crosses to an allowlisted peer](docs/decisions/2026-08-22-federated-media-input-assets.md), which also records what stays refused: model weights and multi-step chain state) — and explicit Eidoverse guest conversations, where submitted world-chat messages and live replies may cross an enabled registered peer under guest admission and the independent default-off Mind travel grant (ADR [Eidoverse guest chat](docs/decisions/2026-09-05-eidoverse-guest-chat.md)). No private records, secrets, automatic conversation-history exports, or record-bearing status payloads are authorized by those exceptions.
...
- `createCollectionStore` (`server/lib/collectionStore.js`) — when a service outgrows its single-JSON-file shape (large per-record payload, frequent mutations), use this instead of another `readJSONFile` + `atomicWrite` + `createFileWriteQueue`. Lays out `data/{type}/{id}/index.json` with a type-level `data/{type}/index.json` stamping the storage-layout `schemaVersion`; includes a per-id write queue and a `verifySchemaVersion` hook used by the boot-time verifier in `server/index.js`. Worked example: `server/services/universeBuilder.js` (migration 034).
...
- `formatCount` / `formatUsd` (`client/src/utils/formatters.js`) — thousands-grouped, en-US-pinned (`2,762`, `$4,610.09`). Reach for these rather than rendering a raw integer or a bare `.toLocaleString()` for a user-facing count or amount — an unseparated `4610` is read at the wrong order of magnitude, and the browser locale disagrees with the `$` beside it. A **measured** zero already renders `0`; `formatCount` gives `—` only for a value that is missing or unparseable, so pass `{ fallback: '0' }` just where an ABSENT count should still read as zero. Converging the ~80 remaining `.toLocaleString()` renders and adding the guard that enforces this is tracked in #7433, so expect to still find both spellings in the tree.
...
- **Capture deferred work, and decide rather than park it.** Deferred refactors/cleanups go into a filed GitHub issue labeled `plan`, specific enough to pick up cold — never left only in chat. When the sole obstacle is an undecided design choice, **make the call yourself** and file it ready-to-work; `future` / `needs-input` are last resorts. Invoke the `portos-file-issue` skill for the labeling contract: independent dispatch hints (`model:light|medium|heavy`, `effort:low|medium|high|xhigh|max`) and contributor labels (`good first issue`, `help wanted`) only when the work you just inspected justifies them — omit an axis rather than guess, never stamp `medium` on both by reflex, never mark a wide mechanical sweep a good first issue — plus `planner:<model>` taken verbatim from the "Planner Attribution" section of your own prompt (omit it when your run was given none). Create missing labels lazily with slashdo/GitHub colors before `gh issue create`, use repeated `--label` flags, and keep category labels (`plan`, `ux`, `bug`, `tests`) intact.
