...
- **Release quality snapshot**: Before a release, run `npm run quality:snapshot` on the install holding PortOS audit evidence and include `quality-snapshot.json` in the release commit. This publishes numeric local assessments only; no provider calls or private run prose. If no local evidence exists, retain the existing snapshot and report that it was not refreshed.
