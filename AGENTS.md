...
Because both are off by default, **never treat "the password is set" as an available guarantee** — gate on it explicitly, or design for the default posture. **PII must not ride the federation layer at all** — ADR [privacy records machine-local](docs/decisions/2026-08-08-privacy-records-machine-local.md). That governs *records* and *status/capability payloads*. The only carve-outs are a submitted job body carrying the conditioning an allowlisted peer is asked to render — the prompt (ADR [federated visual prompts](docs/decisions/2026-08-20-federated-visual-prompts.md)) and, under the same gate, lyrics and conditioning images (ADR [conditioning crosses to an allowlisted peer](docs/decisions/2026-08-22-federated-media-input-assets.md), which also records what stays refused: model weights and multi-step chain state) — explicit Eidoverse guest conversations, where submitted world-chat messages and live replies may cross an enabled registered peer under guest admission and the independent default-off Mind travel grant (ADR [Eidoverse guest chat](docs/decisions/2026-09-05-eidoverse-guest-chat.md)) — and an explicitly promoted Eidoverse foundation's promote envelope, which a registered outbound-enabled peer may pull, style-free and re-verified on both ends (ADR [federated Eidoverse foundations](docs/decisions/2026-09-18-federated-eidoverse-foundations.md)). No private records, secrets, automatic conversation-history exports, or record-bearing status payloads are authorized by those exceptions.
