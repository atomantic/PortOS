...
# Vendored from https://github.com/newideas99/ultra-fast-image-gen
# (quantized_flux2.py) so we can rehydrate aydin99/FLUX.2-klein-4B-int8 without
# pulling that whole project as a dependency.
"""
QuantizedFlux2Transformer2DModel — wrapper that loads an int8/optimum-quanto
quantized Flux2 transformer back into a `Flux2Transformer2DModel` shape that
`diffusers.Flux2KleinPipeline` accepts as `transformer=...`.

The shim is needed because optimum-quanto stores its quantization map in a
`quanto_qmap.json` sidecar instead of the standard diffusers config, so the
default `from_pretrained` path can't reconstruct the model.
"""
