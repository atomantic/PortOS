/** Add the verified AA v4.3 snapshot without replacing an install's researched observations. */
...
export default {
  async up({ rootDir }) {
    ...
    const additions = seed.observations.filter(row => row.id.startsWith('aa-v4.3-') && !ids.has(row.id));
