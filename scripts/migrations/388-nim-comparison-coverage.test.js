...
import migration from './388-nim-comparison-coverage.js';
...
it('upgrades an existing catalog without losing researched evidence and fails closed on future versions', async () => {
  rootDir = await mkdtemp(join(tmpdir(), 'portos-nim-seed-'));
  ...
  const nim = seed.observations.filter(row => row.provider === 'NVIDIA NIM');
  expect(nim).toHaveLength(2);
  expect(nim.map(row => row.model).sort()).toEqual(['google/gemma-4-31b-it', 'poolside/laguna-xs-2.1']);
  expect(nim.every(row => row.quality === null && row.tokensPerSecond === null && row.inputPerMillion.value === 0)).toBe(true);
  const researched = { ...nim[0], notes: 'Example locally researched evidence' };
