/** Add the shipped agentic-coding benchmark rows without replacing an install's researched observations. */
...
export default {
  async up({ rootDir }) {
    ...
    const additions = seed.observations.filter(row => row.id.startsWith('swe-bench-2026-09-16-') && !ids.has(row.id));
