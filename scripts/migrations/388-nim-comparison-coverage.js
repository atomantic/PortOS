/** Add shipped NVIDIA NIM free-endpoint evidence without replacing an install's researched observations. */
...
export default {
  async up({ rootDir }) {
    ...
    const additions = seed.observations.filter(row => row.id.startsWith('nim-free-2026-09-') && !ids.has(row.id));
