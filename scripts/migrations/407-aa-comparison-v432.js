/** Add the September 22 Artificial Analysis v4.3.2 snapshot without replacing researched observations. */
...
export default {
  async up({ rootDir }) {
    ...
    const additions = seed.observations.filter(row => row.id.startsWith('aa-v4.3.2-') && !ids.has(row.id));
