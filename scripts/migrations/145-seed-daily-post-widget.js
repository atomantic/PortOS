/**
 * Seed the new `daily-post` dashboard widget ("Daily POST") into the built-in
 * `health` layout for installs that already have a persisted
 * `data/dashboard-layouts.json`.
 *
 * Background:
 *   `server/services/dashboardLayouts.js#DEFAULT_LAYOUTS` only seeds when the
 *   file is missing. Adding a widget to the Health layout in code alone won't
 *   reach existing users — this migration walks the file, finds the built-in
 *   `health` layout, and inserts `daily-post` into its `widgets` list + `grid`
 *   if missing. User-renamed copies and other layouts are not touched. Re-runs
 *   detect the widget is already present and skip. Mirrors migration 070.
 */

import { readLayoutsDoc, writeLayoutsDoc } from './_lib.js';

const WIDGET_ID = 'daily-post';
const WIDGET_W = 3;
const WIDGET_H = 2;

// Mirror of the geometry in `server/services/dashboardLayouts.js`
// DEFAULT_LAYOUTS `health` layout — edits here must match that file or fresh
// installs + migrated installs diverge.
const PREFERRED_SLOTS = {
  health: { x: 9, y: 3, w: WIDGET_W, h: WIDGET_H },
};
...
export default {
  async up({ rootDir }) {
    const result = await readLayoutsDoc({ rootDir, label: 'migration 145' });
    ...
    if (touched === 0) {
      console.log(`📦 migration 145: daily-post already present in target layouts.`);
      return { updated: 0, reason: 'already-applied' };
    }

    await writeLayoutsDoc(path, doc);
    console.log(`📦 migration 145: seeded daily-post widget into ${touched} built-in layout(s).`);
