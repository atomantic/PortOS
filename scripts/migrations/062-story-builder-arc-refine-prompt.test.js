...
import migration from './062-story-builder-arc-refine-prompt.js';

const STAGE = 'story-builder-arc-refine';
const readJson = (path) => JSON.parse(readFileSync(path, 'utf-8'));

describe('migration 062 — seed Story Builder plot-arc-refine prompt', () => {
  let rootDir;
  let installedStagesDir;
  let installedConfigPath;

  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-062-'));
  ...
  it('is idempotent and preserves a user-customized installed prompt', async () => {
    ...
    writeFileSync(customPath, '# CUSTOM arc refine\n');
    await migration.up({ rootDir });
