...
const FILENAME = 'pipeline-script-verify.md';
const STAGE_KEY = 'pipeline-script-verify';
...
export default {
  async up({ rootDir }) {
    ...
    if (exists) {
      console.log(`📝 pipeline-script-verify prompt: already present`);
    } else {
      const sampleExists = await access(samplePath, constants.F_OK).then(() => true, () => false);
      if (!sampleExists) {
        console.warn(`⚠️  pipeline-script-verify: sample missing for ${FILENAME} — skipping copy`);
      } else {
        try {
          await copyFile(samplePath, dataPath);
          console.log(`✅ seeded ${FILENAME}`);
        } catch (err) {
          console.warn(`⚠️  pipeline-script-verify: copy failed for ${FILENAME}: ${err.message}`);
        }
      }
    }
    ...
    if (!sampleConfigExists) {
      console.warn('⚠️  pipeline-script-verify: data.reference stage-config.json missing — cannot resolve entry; skipping config write');
      return;
    }
    try {
      ...
      if (installed.stages[STAGE_KEY]) {
        console.log(`📝 pipeline-script-verify stage-config: already present`);
        return;
      }
      if (!sample?.stages?.[STAGE_KEY]) {
        console.warn(`⚠️  pipeline-script-verify: sample stage-config missing ${STAGE_KEY} — skipping`);
        return;
      }
      ...
      console.log(`📝 pipeline-script-verify stage-config (${action}): 1 added`);
    } catch (err) {
      console.warn(`⚠️  pipeline-script-verify: stage-config merge failed: ${err.message}`);
    }
  },
};
