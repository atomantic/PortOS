/** Add sourced Grok 4.7 pricing rows without replacing an install's researched observations. */
...
export default {
  async up({ rootDir }) {
    ...
    const additions = seed.observations.filter(row => row.id.startsWith('xai-grok-4.7-pricing-2026-09-21-') && !ids.has(row.id));
