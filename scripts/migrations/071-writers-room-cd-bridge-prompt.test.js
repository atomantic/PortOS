...
import migration from './071-writers-room-cd-bridge-prompt.js';

const STAGE = 'writers-room-cd-bridge';
const readJson = (path) => JSON.parse(readFileSync(path, 'utf-8'));

describe('migration 071 — seed Writers Room CD-bridge prompt', () => {
  let rootDir;
  let installedStagesDir;
  let installedConfigPath;

  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-071-'));
  ...
  it('is idempotent and preserves a user-customized installed prompt', async () => {
    ...
    writeFileSync(customPath, '# CUSTOM bridge\n');
    await migration.up({ rootDir });
