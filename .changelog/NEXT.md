...
- **`findBibleEntryByName(list, name)`** added to `server/lib/storyBible.js` for the "match name OR aliases case-insensitively" predicate. Replaces inlined `.find(...)` in `universeBuilderPromote.js` (duplicate-collision check) AND the seen-set construction in `foldRetiredCharactersBucket` in `server/services/universeBuilder.js` — the per-iteration live-lookup against the mutating array also handles dedup-within-batch implicitly, so the seen-set is gone.
