...
## Agent Operations — one list of workflows

- **[issue-3108] The same workflow now behaves the same everywhere you launch it.** An app's Agent Operations buttons and the CoS task form's Quick Templates were drawing from separate lists that had drifted: some workflows appeared in one place but not the other, the same workflow was described differently depending on where you read it, and one of them was configured to be reviewed twice when queued from a button. Both surfaces now read one shared list, so a workflow's name, description, and run settings are identical wherever it appears.
- **Three more workflows are launchable from an app's Overview** — Plan a Task, Prune Dependencies, and Safety Scan, which were previously only reachable from the task form's templates. The full set is now Plan a Task, Ship Next Issue, Replan Backlog, Review Changes, Push Work, Cut a Release, DevSecOps Audit (SwiftUI variant on Swift apps), Prune Dependencies, and Safety Scan.
- **A workflow queued from an app button is now invoked correctly for whichever agent runs it.** These tasks previously assumed Claude Code, naming the workflow as `/do:review` in the queue row and pasting in Claude's command form. They now store the workflow's name and let the launch step render the right form — `/do:review` for Claude Code, `/do-review` for OpenCode, an Agent Skill for Codex/Grok/Antigravity — the same way the Quick Templates already did.
- **A Claude agent installed at a custom path, or under a renamed provider, is no longer told two contradictory things at once.** Its finishing instructions are now decided by which agent binary actually launches rather than a fixed list of provider names — so such an agent gets the `/simplify` → `/do:pr` sequence it can genuinely run, and conversely a Codex or Gemini agent is no longer handed `/do:pr` commands its CLI doesn't have.
