...
- Local LLMs: when an Ollama model pull fails with `412: requires a newer version of Ollama`, PortOS now offers an inline "Upgrade Ollama & Retry" prompt instead of surfacing the raw error. Runs `brew upgrade ollama` on macOS / re-runs the official Ollama install script on Linux, restarts the Ollama service so the new binary is actually serving requests, and automatically retries the original model download.
