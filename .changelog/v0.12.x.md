...
- **Task-type-specific prompt templates**: New `data/prompts/skills/` directory with specialized agent briefings per task category (bug-fix, feature, security-audit, refactor, documentation, mobile-responsive) — each with routing descriptions, embedded examples, and task-specific guidelines to improve agent accuracy
...
- **Negative example routing**: Anti-pattern recording in task learning so failed model/task-type combinations are tracked and auto-avoided, with routing accuracy surfaced in the Learning tab
- **Deterministic workflow skills**: Recurring autonomous jobs (daily briefing, git maintenance, security audit, app improvement) converted from inline prompts to versioned, editable skill templates with defined steps, outputs, and success criteria
