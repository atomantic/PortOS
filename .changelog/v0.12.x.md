...
### CyberCity — Lighting Controls, Sky Sun & Depth Fog
- **Fixed ambient brightness slider**: Slider now visibly changes scene brightness in real-time — all lights (ambient, point, animated, searchlight, sun directional) use refs + `useFrame` to read brightness every frame, ensuring immediate response to slider changes
- **Replaced FogExp2 with depth fog post-processing**: Three.js `FogExp2` made distant buildings transparent instead of foggy. Replaced with a custom GLSL depth fog shader that reads the depth buffer and blends distant pixels toward a dark blue fog color (`#06101a`), producing correct volumetric-style fog. Fog density slider controls near/far fog range; setting to 0 disables fog entirely
- **Cyberpunk sunset sky dome**: New `CitySky.jsx` component with a large inverted sphere using a custom gradient shader — deep indigo zenith, purple mid-sky, warm magenta/orange horizon — with animated noise ripple for living atmosphere
- **Glowing sun on horizon**: Emissive amber sphere with additive-blended glow halo ring, sitting low on the horizon for a perpetual cyberpunk sunset look
- **Sun directional light**: Real directional light emanating from the sun's position illuminates building faces with warm tone, intensity scaled by ambient brightness slider
- **Fog slider label**: Renamed "FOG DENSITY" to "FOG" for clarity

**New files**:
- `client/src/components/city/CitySky.jsx` — Sky dome gradient shader + sun mesh + directional light

**Modified files**:
- `client/src/components/city/CityLights.jsx` — Rewritten with refs + useFrame for reactive brightness
- `client/src/components/city/CityEffects.jsx` — Added DepthFogShader pass with depth texture render target
- `client/src/components/city/CityScene.jsx` — Removed fogExp2, added CitySky
- `client/src/components/city/cityConstants.js` — Added sky/sun color constants
- `client/src/components/city/CitySettingsPanel.jsx` — Renamed fog slider label
