...
- **Color-Coded Feedback**: Green (≥70% success), yellow (40-69%), and red (<40%) indicators help users quickly assess task type reliability
...
- **Cleaner logs**: Startup logs no longer flood with "📡 Emitted agent:completed" for each orphaned agent - now shows a single summary line like "📡 Emitted agents:orphaned (374 agents)"
...
- **Block instead of re-queue**: When a user terminates or force-kills a running agent, the associated task is now marked as `blocked` (with reason "Terminated by user") instead of being set back to `pending` — prevents killed tasks from automatically restarting on the next evaluation cycle
...
- **Dynamic model discovery**: Embedding model ID is no longer hardcoded — the system detects the actual loaded/available model and uses its correct identifier
