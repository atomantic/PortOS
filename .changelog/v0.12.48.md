...
- **Daily Task Trends Chart**: Visual bar chart showing daily task completions over the last 30 days with success rate color coding, rolling averages, and trend analysis (volume/success rate trending up/down/stable)

**Server changes**:
- `server/services/productivity.js` — New service with streak calculation, pattern analysis, milestone tracking, daily history tracking, and trend calculations
- `server/routes/cos.js` — Added `/productivity`, `/productivity/summary`, `/productivity/recalculate`, `/productivity/trends` endpoints

**Client changes**:
- `client/src/components/cos/tabs/ProductivityTab.jsx` — Full-featured productivity dashboard with collapsible sections
- `client/src/components/cos/DailyTrendsChart.jsx` — Interactive bar chart component showing 30-day task completion history with tooltips, trend indicators, and success rate legend
...
- **Twinkling starfield**: Enhanced sky with 1500 color-varied stars using a custom shader — soft circular points, per-star twinkle animation, and fog-immune rendering so stars are visible through the atmospheric haze
- **Full building names**: App names display in full on building facades using intelligent word-wrapping — hyphens, underscores, and dots are replaced with spaces (e.g., "void-server" renders as "VOID SERVER" across two lines)
- **Visible archive district**: Archived buildings are taller, brighter, and closer to the main city — upgraded from near-invisible to a clearly distinguishable warehouse district with brighter slate wireframes and always-on holographic labels
- **Pixel art window murals**: Each building displays a unique pixel art icon (heart, server rack, lightning, star, shield, gear, globe, terminal) rendered as lit office windows on the building face — deterministic per app via name hash
- **Floating holographic decorations**: Animated 3D wireframe shapes hover above each building rooftop — diamonds, inverted pyramids, Saturn spheres, stacked rings, cubes, and beacon orbs — slowly rotating and bobbing with accent-colored glow
...
- `client/src/components/city/Building.jsx` — App building mesh with neon wireframe edges, pixel art window murals, and hover glow
- `client/src/components/city/BuildingHologram.jsx` — Animated rooftop holographic decorations (6 shape types)
...
### CyberCity — Dynamic City Life Enhancements
- **Animated data streams**: Glowing data packets fly between connected downtown buildings as light trails, visualizing network connectivity between apps — packets travel both directions at varying speeds with sine-arc trajectories and faint connection lines
- **Hover traffic**: Flying hover vehicles cruise around the city at multiple altitudes — perimeter patrol cars circle the downtown district while cross-city lanes slice through at higher elevations, each with engine glow, tail lights, and tilting turns
- **Neon road grid**: Animated road network with dark tarmac strips, glowing edge lines, and traveling light pulses laid out between building rows and columns — distinct neon accent colors per road segment
- **CRT scanline overlay**: Subtle retro-futuristic CRT effect layered over the entire view — horizontal scanlines, radial vignette darkening at edges, and faint chromatic aberration glow for maximum cyberpunk atmosphere
- **Building click navigation**: Clicking any building in the 3D city navigates to the Apps page for quick management access

**Client changes**:
- `client/src/components/city/CityDataStreams.jsx` — Animated data packet trails between connected buildings
- `client/src/components/city/CityTraffic.jsx` — Flying hover vehicles with perimeter and cross-city flight paths
- `client/src/components/city/CityRoads.jsx` — Neon-lit road grid with animated light pulses
- `client/src/components/city/CityScanlines.jsx` — CSS-based CRT scanline, vignette, and chromatic aberration overlay
- `client/src/components/city/CityScene.jsx` — Integrated new scene layers with position data flow
- `client/src/components/city/BuildingCluster.jsx` — Added position callback for data stream/traffic/road coordination
- `client/src/pages/CyberCity.jsx` — Added scanline overlay, building click navigation via React Router

### CyberCity — Immersive Atmosphere & Live Intel
- **Dynamic weather system**: Rain particles and lightning flashes respond to system health — clear skies when all apps are online, increasing rain intensity as more systems go down, with dramatic lightning flashes during critical states. Rain uses custom GPU shaders for elongated drop effects with depth-aware sizing
- **Shooting stars**: Occasional bright meteors streak across the sky with glowing heads and fading trails — 3 staggered slots spawn at random intervals with randomized colors (white, blue-white, golden, cyan), directions, and speeds for a living sky that rewards idle watching
- **Holographic billboards**: Floating neon-framed displays positioned around the downtown perimeter showing rotating real-time system data — online/total counts, uptime percentage, CoS engine status, today's completed tasks, and active work streak. Messages cycle every 6 seconds with gentle bobbing animation
- **Enhanced HUD vitals panel**: New "VITALS" section below the clock showing session uptime, weather status (derived from system health ratio: CLEAR/FAIR/OVERCAST/RAIN/STORM), active agent count, stopped/archived counts, today's task completions, and work streak — all updating in real-time with color-coded status indicators
...
**Client changes**:
- `client/src/components/city/CityWeather.jsx` — GPU shader rain particles with health-reactive intensity + lightning flash system
- `client/src/components/city/CityShootingStars.jsx` — Staggered shooting star spawner with glowing head meshes and shader trail lines
- `client/src/components/city/CityBillboards.jsx` — Floating holographic billboards with neon frames, cycling messages, and support poles
- `client/src/components/city/CityHud.jsx` — Added VITALS panel with uptime, weather, agents, stopped, archived, tasks, and streak stats
- `client/src/components/city/CityScene.jsx` — Integrated weather, billboards, and shooting stars into the 3D scene
- `client/src/pages/CyberCity.jsx` — Added productivity data fetching and prop threading to scene and HUD

### Digital Twin — Genome Section
...
- **Privacy-first**: All data stored locally on the server — genome file never leaves the machine

**Server changes**:
- `server/lib/curatedGenomeMarkers.js` — Curated SNP database with 34 markers across 13 categories, `classifyGenotype()` and `formatGenotype()` functions
- `server/lib/genomeValidation.js` — Zod schemas for upload, search, save, and notes update
- `server/services/genome.js` — Core service with lazy-loaded in-memory Map index, upload/scan/search/CRUD/delete operations
- `server/services/clinvar.js` — ClinVar sync pipeline: streaming download of variant_summary.txt.gz from NCBI, gunzip + line-by-line parse, filtered index of actionable SNPs, genome cross-reference scan
- `server/routes/genome.js` — 12 REST endpoints mounted at `/api/digital-twin/genome` (including 4 ClinVar endpoints)

**Client changes**:
- `client/src/components/digital-twin/tabs/GenomeTab.jsx` — Main tab with upload drop-zone, stats bar, curated markers view, SNP search, and delete confirmation
- `client/src/components/digital-twin/GenomeCategoryCard.jsx` — Reusable collapsible card for marker category groups with status summary
- `client/src/components/digital-twin/constants.js` — Added Genome tab with Dna icon
- `client/src/services/api.js` — Added 8 genome API functions

### CyberCity — Dynamic Sky & Time-of-Day System
- **Time-of-day presets**: New "Time of Day" setting with 4 presets — Sunrise, Noon, Sunset, Midnight — each with unique sky colors, sun/moon position, and scene lighting intensity
...
- **Cyberpunk sky dome**: Custom GLSL gradient shader on an inverted sphere — deep indigo zenith, purple mid-sky, warm magenta/orange horizon at sunset; brighter blue tones at noon; near-black at midnight — with animated noise ripple for living atmosphere
...
- **Daylight-reactive scene lighting**: All scene lights (ambient, point, animated, searchlight) multiply by the preset's `daylightFactor` — noon is bright (1.8x), sunset/sunrise are moderate (0.6x), midnight is dim (0.3x)
- **Fixed ambient brightness slider**: Slider now visibly changes scene brightness in real-time — all lights use refs + `useFrame` to read brightness every frame, combined with daylight factor for immediate response
- **Removed fog**: Fog feature removed entirely — Three.js fog approaches didn't produce the desired visual effect

**New files**:
- `client/src/components/city/CitySky.jsx` — Sky dome gradient shader + celestial body mesh + directional light with hour-based arc

**Modified files**:
- `client/src/components/city/CityLights.jsx` — Rewritten with refs + useFrame, daylight factor from timeOfDay presets
- `client/src/components/city/CityEffects.jsx` — Removed depth fog shader, simplified pipeline
- `client/src/components/city/CityScene.jsx` — Removed fog, added CitySky
- `client/src/components/city/cityConstants.js` — Added timeOfDay presets with sky colors, hours, daylight factors
...
### Brain — Sharpened Pump.fun Project Next Action
- **Specific & time-bound**: Updated the Pump.fun Launch Tracking Engine project's next action from a vague "draft architecture" (stale 33 days) to a concrete deliverable: "Identify 3 candidate data sources and document their rate limits, auth requirements, and response schemas by Feb 20"
...
- **API endpoint**: `POST /api/brain/inbox/:id/done` sets status to `done` with a `doneAt` timestamp, usable by both the UI and the CoS agent
- **Done section**: Collapsed "Done" section at the bottom of the inbox shows completed items with strikethrough text, destination badges, and memory links
- **Needs Review done**: Items in Needs Review can be marked done without filing — useful for fleeting thoughts that don't need to be stored

**Server changes**:
- `server/lib/brainValidation.js` — Added `done` to `inboxStatusEnum`
- `server/services/brainStorage.js` — Added `done` count to `getInboxLogCounts()`
- `server/services/brain.js` — Added `markInboxDone()` function
- `server/routes/brain.js` — Added `POST /inbox/:id/done` endpoint

**Client changes**:
- `client/src/services/api.js` — Added `markBrainInboxDone()` API function
- `client/src/components/brain/constants.js` — Added `done` status color
- `client/src/components/brain/tabs/InboxTab.jsx` — Done buttons on Filed and Needs Review entries, collapsible Done section

## Fixes

### ClinVar Header Parsing — Zero Variants Parsed
- **Fix**: ClinVar variant_summary.txt.gz header parsing stripped `#` from all column names, turning `RS# (dbSNP)` into `RS (dbSNP)`, so the rsid column lookup was always undefined — resulting in 0 matches from 8.6M lines
...
### Codex CLI Provider — Wrong Argument Convention
...
- **Solution**: Added `buildCliArgs()` function that constructs provider-specific arguments — Codex uses `exec --model <model> -` (stdin marker), Gemini uses bare args, and Claude retains `-p -`
...
### AI Toolkit — Provider/Model Logging on Run Creation
- **Run creation log**: The AI toolkit now logs the provider name and model at the start of every AI run (e.g., `🤖 AI run [agent-content-post]: LM Studio/openai/gpt-oss-20b`), making it easy to trace which provider handled each generation in server logs

**Toolkit changes**:
- `portos-ai-toolkit/src/server/runner.js` — Added `console.log` in `createRun()` after provider resolution

### Autonomous Jobs — Daily Briefing Not Running at Scheduled Time
- **Fix**: Autonomous jobs used a simple 24-hour interval from `lastRun` with no concept of time-of-day scheduling — the daily briefing would run whenever 24 hours elapsed, not at a specific morning hour
...
### Agents — Overview Quick Actions & Compact Layout
- **Quick Actions card**: One-click Engage Now and Check Posts directly from the Overview tab — no more navigating to Tools, selecting an account, and finding the button
...
- **Two-column layout**: Overview now uses a responsive 3/2 grid — personality form on the left, quick actions + AI providers + accounts on the right
...
**Client changes**:
- `client/src/components/agents/tabs/OverviewTab.jsx` — Major rewrite: removed AI regen section, added 2-column grid layout, Quick Actions card with engage/check/generate buttons, compact form fields, rate limit display
- `client/src/components/agents/tabs/ToolsTab.jsx` — Auto-select first active account on mount

### Agents — Per-Function AI Provider/Model Configuration
...
- **Backward compatible**: Agents with the old `{ providerId, model }` format continue to work — per-function overrides take precedence when set
...
**Server changes**:
- `server/lib/validation.js` — Expanded `agentAiConfigSchema` with optional `content`, `engagement`, `challenge` sub-objects
- `server/services/agentActionExecutor.js` — `executePost`/`executeComment` use `aiConfig.content`, `executeEngage`/`executeMonitor` use `aiConfig.engagement`
- `server/routes/agentTools.js` — Publish routes pass `aiConfig.challenge` to client, engage/check-posts use `aiConfig.engagement`
- `server/integrations/moltbook/challengeSolver.js` — Added CLI provider dispatch (same pattern as `agentContentGenerator.js`), removed regex fallback

**Client changes**:
- `client/src/components/agents/tabs/OverviewTab.jsx` — 3-row per-function AI provider/model selector UI
- `client/src/components/agents/tabs/ToolsTab.jsx` — Generate Post/Comment use `aiConfig.content` config

### Dashboard — CoS Productivity Widget
...
**Client changes**:
- `client/src/components/CosDashboardWidget.jsx` — New widget component with 4-stat grid (Today, Streak, Pending, Learning), leverages existing `getCosQuickSummary()` and `getCosLearningSummary()` APIs
- `client/src/pages/Dashboard.jsx` — Imported and rendered CosDashboardWidget above the app grid

### Dashboard — Recent Tasks Panel
...
**Server changes**:
- `server/services/cos.js` — Added `getRecentTasks(limit)` function that returns completed tasks sorted by completion time with formatted durations and relative timestamps
- `server/routes/cos.js` — Added `GET /api/cos/recent-tasks` endpoint with optional `?limit=N` query parameter

**Client changes**:
- `client/src/services/api.js` — Added `getCosRecentTasks(limit)` API function
- `client/src/components/CosDashboardWidget.jsx` — Added collapsible Recent Tasks section with expanded/collapsed states, task list with status icons, and summary dot indicators

### CyberCity — Daytime/Nighttime Lighting Awareness
- **`daylightFactor` preset system**: Each time-of-day preset now includes a `daylightFactor` (0.0–1.0) and `groundColor` that drives all daylight-reactive scene behavior — midnight=0.0, sunrise=0.3, sunset=0.2, noon=1.0
...
- **Neon light dimming**: All 9 neon point lights, animated accent lights, and searchlight scale by `neonScaleRef` = `1 - daylight * 0.7` — cyberpunk neon stays visible at 30% during noon, full intensity at night
...
### CyberCity — Billboard Text Fix & Visual Polish
...
- **Bloom post-processing**: Added `@react-three/postprocessing` with subtle bloom effect (intensity 0.8, luminance threshold 0.3) that makes all neon elements, emissive materials, and light sources glow realistically — the single biggest visual upgrade for the cyberpunk aesthetic
...
- **Holographic scan line effect**: Billboards now have an animated GPU shader overlay with horizontal scan lines, a sweeping beam that scrolls vertically, and edge vignette glow — giving them a proper holographic display look
...
### CyberCity — High-Def Cyberpunk Overhaul
...
- **Volumetric light beams**: Five upward-pointing volumetric light cones at city corners and center with GPU shader-based fade, shimmer, and ground glow discs — plus horizontal scan beams that sweep across the city at different altitudes
...
- **Neon street signs**: Floating pixel-font signs positioned around the downtown perimeter reading PORTOS, SYSTEM ONLINE, CYBER, DIGITAL, NEURAL NET, DATA CORE — some with realistic neon flicker effects, all with glow halos and point lights that illuminate nearby geometry
- **Rooftop antenna arrays**: Every active building now has a rooftop antenna with one of 4 types (spire, dish, cross-bar, rotating array), each with a blinking red or accent-colored tip light — adds vertical detail and makes the skyline feel like a real communications hub
...
- **Enhanced particles**: Doubled sparkle counts (120 cyan, 50 pink, 35 purple), added orange ground-level dust and blue high-altitude sparkles — 5 particle layers total
...
**Client changes**:
- `client/src/components/city/CityVolumetricLights.jsx` — New volumetric light beam cones + horizontal scan beams with GPU shaders
- `client/src/components/city/CitySkyline.jsx` — New distant skyline with 60 shader-lit buildings around the perimeter
- `client/src/components/city/CityDataRain.jsx` — New Matrix-style cascading data glyph columns
- `client/src/components/city/CityEmbers.jsx` — New rising ember/spark particles with wind drift
- `client/src/components/city/CityNeonSigns.jsx` — New neon street signs with flicker effects and glow halos
- `client/src/components/city/Building.jsx` — Added rooftop antennas (4 types), vertical neon edge strips, enhanced window textures
- `client/src/components/city/CityGround.jsx` — Added wet puddle patches and ground fog layer
- `client/src/components/city/CityLights.jsx` — Stronger lights, 2 additional accent lights, extended fog range
- `client/src/components/city/CityParticles.jsx` — Doubled counts, added 2 new particle layers
- `client/src/components/city/CityHud.jsx` — Glitch title, health bar, scan line, corner decorations, neon shadows
- `client/src/components/city/CityScanlines.jsx` — CRT flicker, stronger vignette, edge glow lines
- `client/src/components/city/CityScene.jsx` — Integrated all 5 new scene layers, extended orbit range

### M40: Agent Skill System — In Progress
- **Task-type-specific prompt templates** ✅: New `data/prompts/skills/` directory with 6 specialized agent briefings per task category (bug-fix, feature, security-audit, refactor, documentation, mobile-responsive) — each with routing descriptions, embedded examples, and task-specific guidelines. `buildAgentPrompt()` now detects skill type via keyword matching and injects the matched template into the agent briefing. Templates only load when matched to avoid token inflation.
- **Agent context compaction** ✅: Context-limit and output-limit error patterns now include compaction metadata (output size, retry hints) that propagates through task retry metadata. When an agent retries after a context-limit failure, the briefing prompt includes a "Context Compaction Required" section with mandatory output constraints. Agent briefing template updated with task-type-specific output format constraints for documentation, refactors, security audits, and bug fixes.
- **Negative example routing** ✅: Anti-pattern recording in task learning — every task completion now records a `routingAccuracy` cross-reference (taskType × modelTier) with success/failure counts. `suggestModelTier()` enhanced to avoid tiers with <40% success and prefer proven-successful tiers. New "Routing Accuracy" section in Learning tab shows misroutes, tier overview cards, and a full task-type × tier matrix table. New `GET /api/cos/learning/routing` endpoint. Reset also cleans up routing data.
- **Deterministic workflow skills** ✅: Recurring autonomous jobs (daily briefing, git maintenance, brain processing, project review) converted from inline prompt strings to versioned skill templates in `data/prompts/skills/jobs/`. Each template defines structured sections (Steps, Expected Outputs, Success Criteria) that `generateTaskFromJob()` assembles into effective prompts. New API routes (`GET/PUT /api/prompts/skills/jobs/:name`, preview endpoint) and a "Job Skills" tab in the Prompt Manager UI for editing templates without touching code.

### CoS — GitHub Repo Maintenance Scheduler
- **New autonomous job**: `job-github-repo-maintenance` — a weekly recurring CoS job that audits all GitHub repos via `gh` CLI
...
- **Actionable task creation**: Creates prioritized CoS tasks for high-priority repos (5+ security alerts), batch license updates, and dormant repo reviews — not one giant "fix everything" task
...
- **Default disabled**: Ships disabled like other default jobs — enable via the CoS Jobs UI or API

**Server changes**:
- `server/services/autonomousJobs.js` — Added `job-github-repo-maintenance` to `DEFAULT_JOBS` and `JOB_SKILL_MAP`
- `data/prompts/skills/jobs/github-repo-maintenance.md` — New skill template with 7-step workflow
- `data/cos/autonomous-jobs.json` — Added job entry to persistent data

### CyberCity — Immersive Audio & Visual Overhaul
- **Procedural synthwave music**: Web Audio API synthesizer generating ambient cyberpunk music with bass drone, arpeggiated lead, detuned pad layer, LFO filter sweeps, reverb, and delay — chord progression (Am → Em → F → C) at 100BPM, all procedurally generated with zero audio files
- **Spatial sound effects**: 5 synthesized SFX types — building hover (sine sweep), building click (noise burst + ping), lightning thunder (distorted noise), shooting star whoosh (panned noise sweep), and data pulse chirp — all generated in real-time from Web Audio oscillators
- **Enhanced post-processing**: 3 new GPU shader passes on top of existing bloom — chromatic aberration (R/B channel offset at screen edges), animated film grain (noise overlay), and cinematic color grading (shadows → deep blue, midtones → cyan, highlights → warm white)
- **Reflective wet-street ground**: Dark metallic ground plane (metalness 0.9, roughness 0.15) beneath the grid for natural light reflections, doubled puddle count (20 → 40), rolling fog layer at street level with animated opacity
- **Settings system**: localStorage-persisted settings with quality presets (Low/Medium/High/Ultra) controlling DPR, bloom, chromatic aberration, film grain, color grading, particle density, and scanline overlay — all toggleable individually via HUD panel at `/city/settings`
...
**New files**:
- `client/src/hooks/useCitySettings.js` — localStorage-persisted settings hook with quality presets
- `client/src/hooks/useCityAudio.js` — React lifecycle bridge for Web Audio engine
- `client/src/components/city/CitySettingsContext.jsx` — React context provider for settings
- `client/src/components/city/CitySettingsPanel.jsx` — HUD-style settings panel with toggles and sliders
- `client/src/components/city/audio/cityAudioEngine.js` — Singleton Web Audio API manager with master/music/sfx gain buses
- `client/src/components/city/audio/citySynthMusic.js` — Procedural synthwave generator (bass, arp, pad, reverb, delay)
- `client/src/components/city/audio/citySoundEffects.js` — 5 procedural SFX functions

**Modified files**:
- `client/src/pages/CyberCity.jsx` — Settings provider, audio hook, settings panel integration
- `client/src/App.jsx` — Added `/city/settings` route
- `client/src/components/city/CityScene.jsx` — FogExp2, DPR from settings, playSfx prop threading
- `client/src/components/city/CityEffects.jsx` — 3 new ShaderPass effects (chromatic aberration, film grain, color grading)
- `client/src/components/city/CityGround.jsx` — Reflective ground plane, more puddles, rolling fog
- `client/src/components/city/CityLights.jsx` — Animated lights, sweeping searchlight, removed duplicate fog
- `client/src/components/city/CityHud.jsx` — Settings gear icon button
- `client/src/components/city/Building.jsx` — Glass roof, glow halo, SFX on hover/click
- `client/src/components/city/CityWeather.jsx` — Lightning SFX trigger
- `client/src/components/city/CityShootingStars.jsx` — Star spawn SFX trigger
- `client/src/components/city/CityParticles.jsx` — Density multiplier from settings
- `client/src/components/city/CityScanlines.jsx` — Settings toggle
- `client/src/components/city/BuildingCluster.jsx` — playSfx prop threading
- `client/src/components/city/Borough.jsx` — playSfx prop threading

### CyberCity — Borough Layout Redesign
- **Borough neighborhoods**: Each app is now its own borough — a main skyscraper surrounded by smaller process buildings in a ring, creating distinct neighborhoods with organic spacing
...
- **Wider roads**: Road surfaces doubled in width (0.3→0.6) with wider edge line offsets (0.15→0.3) and more padding (0.5→1.5) to match the expanded grid
- **Extended camera**: Camera pulled back (18→25 Y, 30→45 Z), fog far distance extended (90→130), orbit max distance increased (80→120) to accommodate the larger city

**Client changes**:
- `client/src/components/city/ProcessBuilding.jsx` — New lightweight process building component (~8 Three.js objects)
- `client/src/components/city/Borough.jsx` — New wrapper rendering main Building + ProcessBuilding ring + AgentEntity instances
- `client/src/components/city/BuildingCluster.jsx` — Simplified to render Borough per app instead of Building + AgentEntity inline
- `client/src/components/city/cityConstants.js` — Added `BOROUGH_PARAMS` and `PROCESS_BUILDING_PARAMS`, increased spacing to 12.0
- `client/src/components/city/CityScene.jsx` — Adjusted camera position, fog distance, and orbit controls for larger city
- `client/src/components/city/CityRoads.jsx` — Wider road surfaces, edge lines, and padding for larger grid spacing

### Agents — Graceful Moltbook Unavailability Handling
...
- **Warning-level logging**: Platform unavailability logs as `⚠️` warnings instead of `❌` errors with full stack traces, reducing log noise during outages
- **UI warning toasts**: Client shows platform unavailability as a `⚠️` warning toast instead of a red error toast, making it clear the issue is upstream

### Agents — Moltbook Rate Limit Cooldown Timer & 429 Sync
...
- **429 rate limit sync**: When Moltbook returns HTTP 429 (e.g., "You can only post once every 30 minutes"), the server now syncs local in-memory rate limit state to match platform enforcement — fixing the desync that occurs after PM2 restarts when in-memory counters reset to zero
- **Publish error recovery**: Fixed stuck "Publishing..." button state when API calls fail — publish handlers now catch errors gracefully and reset loading state

**Server changes**:
- `server/integrations/moltbook/rateLimits.js` — Added `syncFromExternal()` to update lastAction from external 429 responses
- `server/integrations/moltbook/api.js` — Added `inferActionFromEndpoint()` and 429 detection in `request()` that auto-syncs rate limit state

**Client changes**:
- `client/src/components/agents/tabs/ToolsTab.jsx` — Cooldown timer state, interval tick, formatted countdown in rate limit pills and Publish button, `.catch()` on publish handlers
- `client/src/components/agents/tabs/OverviewTab.jsx` — Matching cooldown timer state and countdown display in Quick Actions rate limit pills

### Brain — Scheduler Initialization & First-Run Catch-Up
- **Scheduler activation**: Brain scheduler (`startBrainScheduler()`) was defined but never called on server startup — daily digests and weekly reviews would never run automatically. Added initialization call in `server/index.js` alongside other service initializers.
- **First-run catch-up fix**: When `lastDailyDigest` or `lastWeeklyReview` was `null` (never run before), the catch-up logic returned `false`, meaning the scheduler would only trigger if the server happened to be running at the exact scheduled minute. Now treats `null` as "never run" and catches up immediately if past the scheduled time.

## Fixes

### Digital Twin — Aesthetic Taste Questionnaire
...
- **5 taste domains**: Movies & Film, Music & Sound, Visual Art & Design, Architecture & Spaces, Food & Culinary — each with 3 core questions and conditional follow-up questions
...
- **AI-powered summaries**: Generate per-section summaries or a unified taste profile using any configured AI provider — extracts core preferences, anti-preferences, key patterns, and representative quotes
...
**Server changes**:
- `server/services/taste-questionnaire.js` — New service with question definitions (5 sections × 3 core + follow-ups), branching logic, answer persistence, and AI summary generation
- `server/routes/digital-twin.js` — 7 new endpoints under `/api/digital-twin/taste/*` (profile, sections, next question, answer, responses, summary, reset)
- `server/lib/digitalTwinValidation.js` — Added `tasteSectionEnum`, `tasteAnswerInputSchema`, `tasteSummaryInputSchema`

**Client changes**:
- `client/src/components/digital-twin/tabs/TasteTab.jsx` — Full conversational Q&A UI with section grid, question flow, review mode, summary generation, and provider selection
- `client/src/components/digital-twin/constants.js` — Added Taste tab with Palette icon
- `client/src/pages/DigitalTwin.jsx` — Imported and rendered TasteTab
- `client/src/services/api.js` — Added 7 taste questionnaire API functions

### Digital Twin — Continuation Prompts for Exhausted Enrichment Dimensions
...
### Digital Twin — Cognitive Decline & Dementia Risk Genetic Panel
- **11 new SNP markers** in a dedicated "Cognitive Decline & Dementia Risk" category covering the most well-replicated Alzheimer's disease risk loci
  - **APOE ε4 (rs429358)**: The strongest common genetic risk factor — 3x risk per allele, 12x for homozygous
  - **APOE ε2 (rs7412)**: Protective counterpart — 40% reduced risk for carriers
  - **BIN1 (rs744373)**: Second strongest AD risk locus after APOE — tau pathology and synaptic function
  - **CD33 (rs3865444)**: Microglial amyloid clearance efficiency
  - **ABCA7 (rs3764650)**: Lipid transport and amyloid processing (1.8x risk)
  - **CR1 (rs6656401)**: Complement receptor 1 — neuroinflammation
  - **CLU (rs11136000)**: Clusterin/apolipoprotein J — amyloid clearance across BBB
  - **PICALM (rs9349407, rs3851179)**: Endocytic clearance and autophagy/tau clearance
  - **CD2AP (rs10948363)**: Blood-brain barrier integrity
  - **MS4A6A (rs983392)**: Microglial activation and TREM2 levels
- **Category card**: Rose-colored category with elderly person emoji, sorted after cardiovascular and tumor suppression

### Digital Twin — Composite APOE Haplotype Determination
- **APOE haplotype resolution**: Genome scan now combines rs429358 (ε4) and rs7412 (ε2) to determine the actual APOE diploid genotype (ε2/ε2, ε2/ε3, ε3/ε3, ε2/ε4, ε3/ε4, ε4/ε4) with accurate risk multipliers and population frequencies
- **Composite marker**: A synthetic "APOE Haplotype" marker appears in the Cognitive Decline category showing the resolved genotype, risk level, and actionable recommendations
- **ε2/ε4 detection**: Properly identifies the rare ε2/ε4 combination (both SNPs heterozygous) where risk and protection partially counteract (~2.6x risk), which individual SNP analysis cannot distinguish
- **ε3 baseline**: Identifies ε3/ε3 carriers (~60% of population) as the reference baseline with standard age-related risk
- **Updated cross-references**: Individual rs429358 and rs7412 marker descriptions now reference each other and the composite haplotype for complete interpretation

### Digital Twin — Genome Dashboard UI Polish
- **Masonry grid layout**: Curated marker category cards now flow in a responsive 1/2/3/4-column masonry grid (via CSS `columns`) instead of a vertical stack, making better use of screen width
- **Wider marker tiles**: Inner marker grid uses 1-2 columns instead of 1-2-3, giving marker names more room to display without truncation
- **Richer category backgrounds**: Category cards use distinct 950-shade backgrounds at 40% opacity (e.g., `bg-purple-950/40`) for clear visual differentiation between categories
- **Softer text contrast**: Marker names and category labels use `text-gray-200` instead of pure white, and genotype badges use `text-gray-300`, reducing eye strain on dark backgrounds
- **Status tags in Markers card**: Moved the beneficial/typical/concern/not-found status breakdown from a standalone row into the Markers metric card, cleaning up the header area

### Digital Twin — Epigenetic Lifestyle Tracking & Neuroprotective Recommendations
- **Curated intervention database**: 12 evidence-based neuroprotective interventions with mechanisms, dosage ranges, evidence levels, and research references — creatine, omega-3 DHA, aerobic exercise, sleep optimization, vitamin D, meditation, B-vitamins, curcumin, cognitive training, social engagement, magnesium L-threonate, and lion's mane
- **Personalized recommendations**: Interventions are filtered by the user's detected genetic marker categories — if you have cognitive_decline markers scanned, you'll see interventions targeting those pathways
- **Daily logging**: Track intake/activity amounts with per-intervention log entries, deduplication by date, and sorted log history
- **Compliance dashboard**: 30-day compliance percentage per intervention with progress bars, streak tracking (consecutive days), and color-coded health indicators
- **Custom interventions**: Add your own supplements or lifestyle practices beyond the curated list with custom units, categories, and frequency
- **Pause/resume/delete**: Toggle interventions active/inactive without losing log history, or remove entirely

**Server changes**:
- `server/services/epigenetic.js` — New service with curated interventions database, CRUD for tracked interventions, daily logging with date dedup, compliance/streak calculation
- `server/lib/genomeValidation.js` — Added Zod schemas for intervention add/update/log
- `server/routes/genome.js` — 7 new REST endpoints for epigenetic tracking (interventions CRUD, logging, recommendations, compliance)

**Client changes**:
- `client/src/components/digital-twin/EpigeneticTracker.jsx` — Full-featured tracking UI with compliance bars, daily logging, curated recommendations with expandable details, custom intervention form
- `client/src/components/digital-twin/tabs/GenomeTab.jsx` — Integrated EpigeneticTracker component between curated markers and ClinVar sections
- `client/src/services/api.js` — Added 7 epigenetic API functions

### Digital Twin — Expanded Curated Genome Marker Database
- **33 new SNP markers** added across 12 new categories and 6 expanded existing categories, bringing the curated database from ~46 to ~79 well-established, GWAS-replicated markers
- **12 new health categories**: Blood Sugar & Diabetes, Gut Health & Digestion, Autoimmune Risk, Thyroid & Hormones, Eye Health, Mental Health, Bone Health, Pharmacogenomics, Cancer Predisposition, Hair Loss, Hearing, Pain Sensitivity
- **6 expanded categories**: Cardiovascular (+PCSK9), Nutrient Metabolism (+FADS1), Inflammation (+CRP), Longevity (+TERC), Sleep & Circadian (+MTNR1B), Skin & UV Response (+TYK2, IL23R)
- **New category colors**: 7 additional Tailwind color variants (teal, pink, fuchsia, lime, slate, stone, zinc) for visual differentiation of the expanded category set

### Digital Twin — Chronotype Gene Markers & Cross-Project Linking
- **2 new chronotype SNP markers** added to the Sleep & Circadian category in the curated genome marker set:
  - **PER2 (rs35333999)**: Core circadian clock gene driving the negative feedback loop; variants affect morning/evening preference and are linked to Familial Advanced Sleep Phase Syndrome
  - **CRY1 (rs2287161)**: Circadian feedback loop repressor; gain-of-function variant lengthens circadian period and is strongly associated with Delayed Sleep Phase Disorder
- **Brain project cross-links**: Genome Section project now documents connections to Chronotype Trait idea (chronotype markers feed chronotype derivation) and Goal Tracker project (longevity/cardiovascular markers feed life-expectancy estimates)
- **Goal Tracker upstream dependency**: Goal Tracker project notes now reference specific genome longevity and cardiovascular markers that will feed adjusted life-expectancy modeling

### Digital Twin — CLI Provider Support for All AI Functions
...
- **Root cause**: These functions only worked with API providers (like LM Studio) — CLI providers (like Claude Code CLI) would silently fail, returning fallback values or errors like "Provider type not supported"
