...
### Identity Orchestrator & Chronotype (M42 P1)
- Added unified Identity API (`GET /api/digital-twin/identity`) that tracks status of all Digital Twin subsystems (Genome, Chronotype, Aesthetics, Goals) in a single endpoint
- Added Chronotype derivation engine that reads 5 sleep markers (CLOCK, DEC2, PER2, CRY1, MTNR1B) and 2 caffeine markers (CYP1A2, ADA) from genome data to classify chronotype as morning/intermediate/evening with confidence scoring
- Weighted genetic scoring: CRY1 (0.30) > CLOCK (0.25) > PER2 (0.20) > MTNR1B (0.15) > DEC2 (0.10), combined with optional behavioral input for composite classification
- Personalized schedule recommendations (wake/sleep times, peak focus windows, exercise windows) based on chronotype
- Caffeine cutoff recommendations based on CYP1A2 fast/slow metabolizer status
- Meal timing recommendations based on MTNR1B variant affecting glucose response
- `PUT /api/digital-twin/identity/chronotype` accepts behavioral overrides (preferred wake/sleep times, focus windows) that blend with genetic data
- `POST /api/digital-twin/identity/chronotype/derive` for force re-derivation after genome updates
