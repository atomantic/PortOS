...
### Browser URL Navigation & Headed Mode
- **Open URL in CDP browser**: Added a URL input bar to the Browser page that opens any URL in a new tab of the managed CDP browser
- **Auto-prefix**: URLs without a protocol are automatically prefixed with `https://`
- **Server endpoint**: New `POST /api/browser/navigate` route with Zod URL validation, using CDP's `/json/new` endpoint to open tabs
- **Conditional visibility**: The URL bar only appears when the browser is connected, keeping the UI clean when the browser is stopped
- **Headed mode support**: Browser now reads the `headless` config setting from `data/browser-config.json` — disable headless to see the browser window and manually authenticate to sites like GitHub
- **Persistent browser profile**: Auth cookies, localStorage, and sessions now persist across restarts via `--user-data-dir` pointing to `data/browser-profile`
- **Config-driven launch**: The browser process reads its configuration from the shared config file on startup instead of hardcoding `headless: true`
- **Live mode display**: Sidebar shows whether the running browser is in headed or headless mode, with a restart hint after config changes
- **Real Chrome instead of Playwright Chromium**: Browser process now spawns system Chrome directly (`/Applications/Google Chrome.app`) instead of Playwright's bundled Chromium, enabling Google sign-in and other auth flows that reject automation browsers
- **Removed Playwright dependency**: Browser process uses `child_process.spawn()` and CDP HTTP endpoints, no longer depends on the playwright npm package
