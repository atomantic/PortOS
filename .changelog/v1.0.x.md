...
- Fixed null pointer in `archiveStaleAgents()` and `cleanupZombieAgents()` when `agent.completedAt` is missing
- Fixed task learning pruning referencing non-existent `stats.lastSeen` (now uses `stats.lastCompleted`)
- Fixed stale closure in AgentsTab `handleDelete` — date bucket count now updates correctly after deletion
- Fixed `saveAgentIndex()` crash safety — now uses atomic temp-file + rename pattern (matching `saveState()`)
- Fixed memory leak in `liveOutputs` — completed agent output buffers are now cleaned up
- Fixed unreachable dead code in productivity streak calculation
