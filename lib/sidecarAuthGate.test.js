...
describe('createSidecarAuthGate', () => {
  ...
  describe('when the install has no password', () => {
    ...
    it('still refuses a cross-site or rebound browser request (#8707)', async () => {
      const { gate } = makeGate();
      for (const [headers, code] of [
        [{ origin: 'https://attacker.example', host: '127.0.0.1:5560' }, 'CROSS_ORIGIN_BLOCKED'],
        [{ origin: 'http://rebind.attacker.example:5560', host: 'rebind.attacker.example:5560' }, 'HOST_NOT_ALLOWED'],
      ]) {
        const { res, nexted } = await run(gate, req('/api/restart/portos-server', headers));
        expect(nexted).toBe(false);
        expect(res.statusCode).toBe(403);
        expect(res.body.code).toBe(code);
      }
      expect((await run(gate, req('/api/restart/portos-server', {
        origin: 'http://localhost:5555', host: 'localhost:5560',
      }))).nexted).toBe(true);
    });
  ...
  describe('when the install is password protected', () => {
    ...
    it('accepts the main UI\'s port-scoped cookie even behind a stale legacy one', async () => {
      await writeSessions([{ tokenHash: hashToken('tok-scoped'), expiresAt: Date.now() + 60_000 }]);
      const { gate } = makeGate();
      const ok = await run(gate, req('/api/apps', { cookie: 'portos_auth=tok-stale; portos_auth_5555=tok-scoped' }));
      expect(ok.nexted).toBe(true);
    });
