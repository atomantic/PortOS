...
export const createSidecarAuthGate = ({
  ...
  const isAuthenticated = async (req) => {
    ...
    // Any PortOS session cookie (legacy or port-scoped — the main UI's
    // `portos_auth_5555` rides along to :5560 because cookies ignore port) or
    // Bearer token; a foreign install's cookie simply doesn't verify.
    for (const token of extractTokens(req)) {
      if (await verifyServerSession(token)) return true;
    }
  ...
  // Host check) runs FIRST and in both modes: without a password any web page
  // the user opens could otherwise drive this sidecar through the user's own
  // browser, and the public logout endpoint still mutates state. Past that, an
  // install with no password passes everything through.
  const gate = async (req, res, next) => {
    const refusal = browserRequestRefusal(req);
    if (refusal) {
      return res.status(403).json({ error: 'Browser request rejected', code: refusal });
    }
