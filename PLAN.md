...
- [ ] **Retire or wire up `StatCard`'s dead `activeLabel` prop, and de-dup the two hand-rolled Learning cards.** `activeLabel` (`client/src/components/cos/StatCard.jsx:1`, rendered at :59-63 and interpolated into `ariaLabel` at :2) has **zero callers** across the client — no `activeLabel=` usage and no spread props into `<StatCard>`. It is also only honored by the *default* variant: the `compact` (:6-27) and `mini` (:29-46) variants silently drop it, so a future caller passing it to a compact card would get nothing. Either delete the prop + its render branch, or make the compact/mini variants honor it. Related: the two Learning cards (`client/src/pages/ChiefOfStaff.jsx` ~512-537 compact, ~832-858 ascii `mini`) are hand-rolled buttons rather than `StatCard`s precisely because StatCard supports neither `onClick` nor a status-driven border color nor a sub-label on the compact variant — and they duplicate the same `status === 'critical' ? … : 'warning' ? …` color ternaries three times each. Wiring the sub-label through (an `activeLabel`-style slot honored by all variants) plus `onClick`/border-tone props would let both collapse onto StatCard. Deferred from the mobile-overflow fix (which only restructured the compact card's value/label stacking): removing or re-speccing a prop on a shared component used by every CoS stat card is a distinct refactor deserving its own PR + tests, and the overflow fix must not ride on it.
