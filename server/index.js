...
// Self-signed TLS: if cert files exist (generated by scripts/setup-cert.js),
// serve HTTPS so getUserMedia and other secure-context APIs work over the
// Tailscale IP. Otherwise fall back to plain HTTP.
const CERT_PATH = join(PATHS.data, 'certs', 'cert.pem');
const KEY_PATH = join(PATHS.data, 'certs', 'key.pem');
const httpsEnabled = existsSync(CERT_PATH) && existsSync(KEY_PATH);
const httpServer = httpsEnabled
