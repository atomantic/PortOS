...
describe('closed, governed recipe definition boundary', () => {
  ...
  it('defaults the definition grant off and preserves it across old capability updates', () => {
    ...
    expect(mergePersistentMindCapabilities({ manageToolRecipes: true }, { schemaVersion: 3, readPortos: false })).toMatchObject({ schemaVersion: 14, auditReports: false, manageToolRecipes: true, readPortos: false });
  });
});
