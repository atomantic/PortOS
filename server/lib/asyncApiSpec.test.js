...
describe('AsyncAPI spec', () => {
  it('builds AsyncAPI 3 channels and direction-aware operations from the socket catalog', async () => {
    const spec = await buildAsyncApiSpec({ baseUrl: 'https://portos.example.test', version: '1.2.3' });
    ...
    expect(Object.values(spec.components.messages).every((message) => !('x-portos-source' in message))).toBe(true);
  });

  it('declares every dynamic channel address parameter', async () => {
    const spec = await buildAsyncApiSpec({ baseUrl: 'http://example.com', version: '1.2.3' });
