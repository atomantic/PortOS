...
| `requestAbort.js` | `abortSignalFromRequest(req)` — AbortSignal that fires when an Express client disconnects, so a streaming handler can tear down its upstream read on cancel. |
