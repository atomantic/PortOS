...
| `tribeMatch.js` | Pure, deterministic matcher mapping a calendar attendee / message counterpart (`{ email, name }`) to a tracked Tribe person for auto-logged touchpoints (#2033): `buildPersonMatchIndex(people)` → `{ byIdentifier, byName }`, `matchPerson(identity, index)` (email/handle authoritative, exact unique name fallback — no fuzzy matching), `matchPeople(identities, index)` → de-duplicated Set of personIds, `normalizeIdentifier(value)`. Consumed by `autoLogTouchpoints` in `services/tribe.js` from the calendar and message sync producers. |
