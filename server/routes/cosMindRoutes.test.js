...
describe('persistent mind routes', () => {
  ...
  it('serves a bounded cursor snapshot with only the safe profile fields', async () => {
    ...
    expect(res.body).toMatchObject({
      ...
      capabilities: { schemaVersion: 12, createTasks: true, fileIssues: false, manageMind: false, manageEidoverse: false, visitEidoversePeers: false, promoteEidoverseFoundations: false, callUser: false, adjustLocalContext: false, readPortos: false, writePortos: false, taskModelAllowlist: [] },
  ...
  it('loads the shared roster for the issue grant alone, so its allowlist edits see every app', async () => {
    mocks.loadState.mockResolvedValue({ config: {
      persistentMindCapabilities: { schemaVersion: 12, createTasks: false, fileIssues: true },
    } });
    mocks.readPersistentMindManagedApps.mockResolvedValue([
