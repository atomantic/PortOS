...
describe('reconcileBeeperIngestion', () => {
  ...
  it('replaces the active transport after connection settings change without resetting the sweep', async () => {
    captureLogs();
    await reconcileBeeperIngestion();
    const firstRun = getEvent('beeper-sync').nextRunAt;

    await reconcileBeeperIngestion({ reason: 'connection-settings', restartSocket: true });
