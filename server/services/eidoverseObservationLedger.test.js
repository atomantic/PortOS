 ...
 * the next observation, a section that failed to collect degrades instead of
...
const { observeEidoverseWorld, readEidoverseObservationMarker } = await import('./eidoverseObservationLedger.js');
...
beforeEach(() => {
  ...
  listControllers.mockReset().mockResolvedValue({ counts: { installed: 0, armed: 0 }, installs: [] });
...
describe('the Eidoverse observation marker', () => {
  it('reads as never-observed on an install whose mind has never looked', async () => {
    expect(await readEidoverseObservationMarker()).toBeNull();
  });
  ...
  it('carries what is new across the wake boundary the marker exists for', async () => {
    ...
    expect(first.markerCommitted).toBe(true);
  ...
  it('leaves the marker untouched when asked to look without stamping', async () => {
    ...
    expect(peek.markerCommitted).toBe(false);
  ...
  it('refuses to treat an unreadable marker file as never-observed', async () => {
    ...
    await expect(readEidoverseObservationMarker()).rejects.toThrow();
  ...
  it('degrades a marker written by a newer PortOS to one honest first observation', async () => {
    ...
    expect((await readEidoverseObservationMarker()).schemaVersion).toBe(1);
  });
