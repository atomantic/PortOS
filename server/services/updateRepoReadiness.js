 ...
 * is the caller: `autoUpdateScheduler.runAutoUpdateTick` stamps the cooldown
 * baseline (`repairQueuedAt`) right after this call returns, so a stand-down
 * agent costs one dispatch per cooldown window rather than one per poll
 * (#7468). The task store is imported lazily — this module is on the update
 * path, the repair branch is rare, and a static import would pull the whole
 * CoS state graph into that closure.
 *
