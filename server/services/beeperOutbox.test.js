...
describe('confirmation — socket first, 30s GET fallback', () => {
  ...
  it('settles a missed confirmation when its deadline fires during a socket lookup', async () => {
    markPriorSend();
    const entry = await approvedEntry();
    await sendOutboxEntry(entry.id);
    let finishLookup;
    getMessage.mockImplementationOnce(() => new Promise((resolve) => { finishLookup = resolve; }));
    listMessagesPage.mockResolvedValue({ items: [] });
    beeperSocketEvents.emit('invalidate', { kind: 'message.upserted', chatID: CHAT_ID });
    clock.advance(CONFIRMATION_TIMEOUT_MS);
    clock.runTimersWithDelay(CONFIRMATION_TIMEOUT_MS);
    finishLookup(null);
    await flush();
    expect(outbox.get(entry.id)).toMatchObject({ state: 'awaiting-confirmation', errorCode: 'CONFIRMATION_UNRESOLVED' });
