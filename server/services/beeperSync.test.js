...
describe('watermark-bounded sweep', () => {
  ...
  // the top of the list.
  it('walks past a caught-up page to resume a truncated chat on an older page', async () => {
    ...
    installFetch({
      chatPages: [
        {
          items: [chatRow('chat-caught-up', '2026-09-02T10:00:00.000Z')],
          hasMore: true,
          oldestCursor: 'chats-page-2',
        },
        { items: [chatRow('chat-truncated', '2026-09-01T00:00:00.000Z'), chatRow('chat-quiet', '2026-07-01T00:00:00.000Z')], hasMore: false },
      ],
      messagePages: {
        'chat-truncated': {
    ...
    // A fully caught-up first page must not hide a backlog on the second.
    expect(chatsRequests()).toHaveLength(2);
  });
