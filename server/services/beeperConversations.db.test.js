 ...
 * this file proves that the indexes can serve ordered reads on a real seeded
 * dataset, alongside service pagination and eviction-order behavior. It does
 * not measure which plan an unrestricted planner chooses for a large mirror.
 *
 * Sequential and bitmap scans are disabled inside the EXPLAIN transaction:
 * on small fixtures PostgreSQL may prefer either a sequential scan or a
 * bitmap scan followed by a tiny sort even when an ordered index exists.
 * Restricting those alternatives does not change what is being asserted — the
 ...
 * sequential and bitmap scans disabled for that transaction alone (`SET LOCAL`
 * resets on COMMIT/ROLLBACK, so it never leaks onto another pooled request).
