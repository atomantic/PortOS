...
import { ZERO_REVIEWER_COVERAGE_NOTE, isProviderReviewer, LOCAL_LLM_REVIEWERS } from '../lib/reviewerConfig.js';
...
export function buildLocalReviewerInstructions(reviewers, reviewerModels = {}, reviewerEfforts = {}, { claimCommentGate = false, enforceIsolation = false } = {}) {
  ...
  // Provider identities may now run tool-enabled harnesses. Only known local
  // chat-completion backends belong in this optional comment-screening stage;
  // otherwise the claim procedure uses its existing deterministic parser.
  const ingressReviewer = localReviewers.find(reviewer => LOCAL_LLM_REVIEWERS.includes(reviewer));
  const ingressPinned = {
    kind: 'claim-comments',
  ...
  const claimCommentGateBlock = claimCommentGate && ingressReviewer ? `
...
\`\`\`` : '';
  ...
  const accessPolicy = enforceIsolation
    ? 'Claim reviews send only the supplied diff. Provider CLIs run in an isolated scratch directory with the diff inlined, under an enforced no-tool or read-only vendor profile when one exists and without forge or cloud credentials. API and local-model providers receive the diff without tools.'
