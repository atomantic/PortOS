...
- The work would require touching files far outside the issue's scope (>5 unrelated files), suggesting it's bigger than a single claim — release it and let \`/claim --issues\` or a human split it into slices.
...
- The request references a function, file, or component that no longer exists (\`grep -rn\` the named identifiers — if they're gone, the issue is stale).
- The work would require touching files far outside the issue's scope (>5 unrelated files) — release it and let a human split it into slices.
