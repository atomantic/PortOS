...
const DEFAULT_LAYOUTS = [
  ...
  },
  {
    ...
    name: 'Morning Review',
    builtIn: true,
    widgets: ['daily-driver', 'proactive-alerts', 'upcoming-tasks', 'review-hub', 'goal-progress', 'death-clock'],
    // Scan-and-act morning ritual. The Daily Driver (#2666) is pinned full-width
    // at the very top — the first-visit-of-day sequence (POST → goal
    // next-actions) — and self-hides once handled, collapsing the layout back to
    // the classic scan quadrants below it. Tasks list takes the tall center
    // column (the actionable hot zone); alerts top-left grab attention first;
    // death-clock top-right for mortality framing; review + goals fill the rest.
    grid: [
      { id: 'daily-driver',     x: 0, y: 0, w: 12, h: 6 },
      { id: 'proactive-alerts', x: 0, y: 6, w: 4, h: 4 },
      { id: 'upcoming-tasks',   x: 4, y: 6, w: 5, h: 8 },
      { id: 'death-clock',      x: 9, y: 6, w: 3, h: 2 },
      { id: 'goal-progress',    x: 9, y: 8, w: 3, h: 4 },
      { id: 'review-hub',       x: 0, y: 10, w: 4, h: 4 },
    ],
  },
  {
    id: 'ops',
