...
import { isAuditTaskType, defaultFileIssuesFor, auditDoWorkRequiresWorktree, getAuditScheduleMetadata, AUDIT_RUN_GUIDANCE } from '../lib/auditCatalog.js';
...
export async function updateTaskInterval(taskType, settings) {
  const { task, unparkedScopes } = await updateSchedule(async (schedule) => {
    ...
    // Labels organize the page; they do not signal that a failed task is repaired.
    const labelsOnly = Object.keys(settings).every(key => key === 'labels');
    if (topExec && !labelsOnly) {
