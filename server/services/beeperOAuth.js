...
// Disconnect invalidates unfinished connects, including a token exchange already
// waiting on the network. Serialize only credential mutation and reconciliation
// so a save already underway finishes before disconnect removes it.
let connectionGeneration = 0;
const queueCredentialMutation = createFileWriteQueue();

function assertConnectionCurrent(generation) {
  if (generation !== connectionGeneration) {
    throw oauthError('This Beeper connection was cancelled — start the connect again', 'OAUTH_STATE_UNKNOWN', { status: 400 });
  }
}

function storeConnectedCredential(input, generation, reason) {
  return queueCredentialMutation(async () => {
    assertConnectionCurrent(generation);
    const saved = await saveBeeperCredential(input);
    await armIngestion(reason);
    return saved;
  });
}
...
async function fetchJson(url, init, { timeoutMs = OAUTH_TIMEOUT_MS } = {}) {
  let response;
  try {
    // A 307/308 can forward the token or PKCE verifier in the POST body to
    // another origin. Endpoint validation must also hold after the request.
    response = await fetchWithTimeout(url, { ...init, redirect: 'error' }, timeoutMs);
  } catch (err) {
...
export async function startBeeperOAuth({ redirectUri, clientUri } = {}) {
  ...
  pendingAuthorizations.set(state, {
    codeVerifier, clientId, redirectUri, createdAt: Date.now(), generation,
...
export async function completeBeeperOAuth({ code, state } = {}) {
  ...
  const scopes = typeof body.scope === 'string' && body.scope.trim()
    ...
    : BEEPER_OAUTH_SCOPES;
  const saved = await storeConnectedCredential({
    token,
    expiresAt: expiryFromTokenResponse(body),
    scopes,
    source: 'oauth',
    clientId: pending.clientId,
  }, pending.generation, 'oauth-connect');
...
async function armIngestion(reason) {
  await reconcileBeeperIngestion({ reason, restartSocket: true })
 ...
 */
export function disconnectBeeper() {
  connectionGeneration += 1;
  pendingAuthorizations.clear();
  return queueCredentialMutation(disconnectCredential);
}
...
async function disconnectCredential() {
  ...
  // Clear the supported legacy fallback before deleting the vault entry;
  // otherwise the resolver resurrects that credential and re-arms ingestion.
  await updateSettingsWith((settings) => {
    if (!isPlainObject(settings.beeper)) return settings;
    const { token, tokenExpiresAt, ...beeper } = settings.beeper;
    return { ...settings, beeper };
  });
