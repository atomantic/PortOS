...
beforeEach(() => {
  ...
  vi.mocked(updateSettingsWith).mockImplementation(async (mutate) => mutate(await getSettings()));
...
describe('completeBeeperOAuth', () => {
  ...
  it('arms ingestion once the credential is stored, without waiting for a restart', async () => {
    ...
    expect(reconcileBeeperIngestion).toHaveBeenCalledWith({ reason: 'oauth-connect', restartSocket: true });
  });
...
describe('connectWithPastedToken', () => {
  ...
  it('arms ingestion after vaulting a pasted token', async () => {
    ...
    expect(reconcileBeeperIngestion).toHaveBeenCalledWith({ reason: 'token-pasted', restartSocket: true });
  });
...
describe('disconnectBeeper', () => {
  it('cancels an outstanding authorization before its callback can reconnect', async () => {
    mockConnectStart();
    const { state } = await startBeeperOAuth({ redirectUri: REDIRECT_URI });
    vi.mocked(readBeeperCredential).mockResolvedValue(null);
    vi.mocked(deleteBeeperCredential).mockResolvedValue({ deleted: false, tokenConfigured: false });

    await disconnectBeeper();

    await expect(completeBeeperOAuth({ code: 'example-code', state })).rejects.toMatchObject({ code: 'OAUTH_STATE_UNKNOWN' });
    expect(saveBeeperCredential).not.toHaveBeenCalled();
  });

  it('cannot restore a token whose exchange finishes after disconnect', async () => {
    mockConnectStart();
    const { state } = await startBeeperOAuth({ redirectUri: REDIRECT_URI });
    let finishExchange;
    let exchangeStarted;
    ...
    const completion = completeBeeperOAuth({ code: 'example-code', state });
    const rejected = expect(completion).rejects.toMatchObject({ code: 'OAUTH_STATE_UNKNOWN' });
    await started;
    vi.mocked(readBeeperCredential).mockResolvedValue(null);
    vi.mocked(deleteBeeperCredential).mockResolvedValue({ deleted: false, tokenConfigured: false });
    await disconnectBeeper();
    finishExchange(mockJsonResponse({ access_token: TOKEN }));

    await rejected;
    expect(saveBeeperCredential).not.toHaveBeenCalled();
  });

  it('waits for an already-started credential save before deleting it', async () => {
    let finishSave;
    let saveStarted;
    ...
    vi.mocked(saveBeeperCredential).mockImplementationOnce(() => {
      saveStarted();
      return new Promise((resolve) => { finishSave = resolve; });
    });
    const connecting = connectWithPastedToken(TOKEN);
    await started;
    vi.mocked(readBeeperCredential).mockResolvedValue({ token: TOKEN, tokenSource: 'pasted' });
    vi.mocked(deleteBeeperCredential).mockResolvedValue({ deleted: true, tokenConfigured: false });
    const disconnecting = disconnectBeeper();
    expect(deleteBeeperCredential).not.toHaveBeenCalled();
    finishSave({ tokenConfigured: true });

    await connecting;
    await expect(disconnecting).resolves.toMatchObject({ tokenConfigured: false });
    expect(reconcileBeeperIngestion.mock.calls.at(-1)).toEqual([{ reason: 'disconnect', restartSocket: true }]);
    expect(deleteBeeperCredential).toHaveBeenCalledOnce();
  });

  it.each([null, { token: TOKEN, tokenSource: 'pasted', clientId: '' }])(
    'removes the legacy fallback before ingestion reconciles, with vaulted credential %j',
    async (credential) => {
      let settings = {
        theme: 'dark',
        beeper: { baseUrl: BASE_URL, enabled: true, token: 'legacy-example-token', tokenExpiresAt: '2027-01-01' },
      };
      vi.mocked(readBeeperCredential).mockResolvedValue(credential);
      vi.mocked(getSettings).mockImplementation(async () => settings);
      vi.mocked(updateSettingsWith).mockImplementation(async (mutate) => {
        settings = await mutate(settings);
        return settings;
      });
      vi.mocked(deleteBeeperCredential).mockResolvedValue({ deleted: Boolean(credential), tokenConfigured: false });

      await expect(disconnectBeeper()).resolves.toMatchObject({ tokenConfigured: false });

      expect(settings).toEqual({ theme: 'dark', beeper: { baseUrl: BASE_URL, enabled: true } });
      expect(updateSettingsWith.mock.invocationCallOrder[0]).toBeLessThan(deleteBeeperCredential.mock.invocationCallOrder[0]);
      expect(deleteBeeperCredential.mock.invocationCallOrder[0]).toBeLessThan(reconcileBeeperIngestion.mock.invocationCallOrder[0]);
    },
  );

  it('reports failure instead of claiming disconnected when legacy credential removal cannot persist', async () => {
    vi.mocked(readBeeperCredential).mockResolvedValue(null);
    vi.mocked(updateSettingsWith).mockRejectedValue(new Error('Settings unavailable'));

    await expect(disconnectBeeper()).rejects.toThrow('Settings unavailable');
  ...
  it('revokes an OAuth credential at the authorization server, then deletes the local copy', async () => {
    ...
    expect(revoke[1].redirect).toBe('error');
  ...
  it('reconciles ingestion after deleting the credential, so the relay stops', async () => {
    ...
    expect(reconcileBeeperIngestion).toHaveBeenCalledWith({ reason: 'disconnect', restartSocket: true });
  });
});
