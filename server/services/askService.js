...
import { resolveCliModel, prefixOpencodeModel, hasModelFlag } from '../lib/providerModels.js';
...
async function* streamCompletion(provider, model, prompt, signal) {
  ...
  if (provider?.command === 'opencode' && !args.includes('run')) args.unshift('run');
  ...
  } else if (provider?.command === 'opencode') {
