...
describe.skipIf(!runDb)('universeBuilder DB adapter round-trip', () => {
  ...
  it('projects names and index counts, with a compatibility fallback only for legacy canon', async () => {
    await db.writeRaw('u-live', U('u-live', {
      logline: 'A city', starterPrompt: 'Treatment '.repeat(1000), characters: [{ id: 'c1', name: 'Ada' }], places: [], objects: [],
      styleImageRefs: ['old.png', 'last.png'], premise: 'Not a summary field',
    }));
    await db.writeRaw('u-legacy', U('u-legacy', { schemaVersion: 3, categories: {}, starterPrompt: 'Legacy treatment '.repeat(1000) }));
    ...
    expect(live.starterPrompt).toHaveLength(500);
    expect(live).not.toHaveProperty('characters');
    expect(live).not.toHaveProperty('premise');
    expect(summaries.find((u) => u.id === 'u-legacy').legacyRecord).toMatchObject({
      schemaVersion: 3, starterPrompt: 'Legacy treatment '.repeat(1000).slice(0, 500),
    });
  });
