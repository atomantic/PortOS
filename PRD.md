...
| NR-4 | The system MUST support the user sharing their own records across machines they own/control through the configured federation channels. Personal or private content alone MUST NOT disqualify a record from that sync. Credentials and machine-local configuration remain local; sharing with other people or services requires the applicable explicit audience controls. | Federation within the user's private network is a product capability, distinct from public disclosure. Preserve existing peer/category controls, wire contracts, and currently local-only store behavior; see [user-controlled federation](docs/decisions/2026-09-19-user-controlled-federation.md). |
