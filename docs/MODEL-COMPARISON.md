...
- `POST /api/providers/comparison/sync/:source` with an optional `{ "apiKey": "..." }` (used only by `artificial-analysis`) → the same import contract for any declared source. `GET` reports the selectable list as `syncSources: [{ id, label, requiresKey }]`. Built-in sources: `artificial-analysis`, `swebench`, and `livecodebench` — see "Benchmark source sync" below.
