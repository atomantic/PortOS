...
1. Upgrade both instances. Open **Instances** from the local interface on each
   machine, or sign in with that machine's instance password for remote settings.
...
Peer configuration changes require an operator session or a direct loopback
connection with a local browser origin while the instance password is off.
