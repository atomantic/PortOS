...
- Eidoverse world foundations ledger — `data/eidoverse/foundations.json` (#7455). The install-local record of every durable world foundation this instance authored: its ownership layer (`vernacular` by default, `baseline` once promoted), the promotable `body`, the vernacular `style` layer that never leaves, provenance (opaque instance id + author kind), the last agent-free resilience verdict, and the last packaged promote candidate. `file-primary` and **intentionally machine-local — never federated**: the only thing authorized to leave the install is a packaged candidate envelope, and only through an explicit promote. No `data.reference/` seed and no migration — an absent file IS the empty ledger every install starts from, so a seed would only put shipped defaults where a user's foundations belong. Backed up with the rest of `data/eidoverse/`.
