...
- `catalog_ingredient_relations` — directed ingredient→ingredient graph edges (the strongest argument for Postgres as the catalog graph store). Since #7615, committing a scrap also mints one `related-to` edge per unordered pair among the ingredients extracted from that scrap in one batch (bounded to batches of 25 or fewer), so a single source's extractions land as a connected cluster instead of isolated nodes.
...
- `review_queue_triage` — the Review Hub's machine-local presentation markers: canonical `action_key` plus `occurrence`/`revision`, `snoozed_until`, optional recommendation `dismissed`, and a delivery `generation` marker reserved for the delivery layer. It contains no queue title, summary, prompt, source payload, or notification-read state; the live producer remains authoritative and a new occurrence/revision gets a fresh presentation decision. **Intentionally machine-local — never federated**, including when the source row is a federated Brain record. PostgreSQL is authoritative and the normal `pg_dump` captures it; the JSON adapter exists only for the documented dev/test file escape hatch. Adapter: `server/services/reviewQueueTriageStore.js`.
