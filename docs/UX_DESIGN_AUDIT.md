...
- Settings General and Backup, Brain Threads and Notes, and the Apps list and detail: current source and rendered-test review only; #7858 and #7860 have synthetic validation evidence, while complete browser geometry and accessibility certification remain out of scope.
...
| --- | --- | --- | --- | --- |
| Settings General — `client/src/pages/Settings.jsx`, `client/src/components/settings/GeneralTab.jsx` | Configuration. Set timezone; set or clear location; choose interface theme. | Before: three independent cards compete equally for attention → after: timezone and location share a responsive recurring-configuration grid, theme follows as a secondary instant preference, and save errors remain beside their field group. | `/settings/general` stays canonical, with the Settings icon from `NAV_PRESENTATION` and legacy tab redirects. The Settings content region owns the normal scroll. Independent saves, dirty indicators, and the unsaved-changes guard remain visible. | Load failure still establishes an empty baseline and toasts; validation and save failures now also render inline, while the affected group stays dirty. Implemented in #7858; synthetic evidence: [validation/7858-settings-workspace.md](validation/7858-settings-workspace.md). |
| Settings Backup — `client/src/pages/Settings.jsx`, `client/src/components/settings/BackupTab.jsx` | Operations + Configuration. Inspect backup health; configure schedule/destination/excludes; run or restore a backup. | Before: status, schedule, exclusions, snapshots, and recovery controls form one long sequence → after: health and saved schedule lead, destination/schedule and saved-state actions follow, and exclusions plus snapshot history are named disclosures. | `/settings/backup` stays canonical with the Download icon. The Settings content region remains the primary scroll owner; the action bar stays sticky without covering errors. Saved destination/schedule state, Run Now gating, dry-run restore, and destructive-action confirmation remain unchanged. | `loadFailed` remains an explicit unavailable panel; degraded database status, invalid restore input, and failed restore remain distinct from an empty snapshot list. Implemented in #7858; synthetic evidence: [validation/7858-settings-workspace.md](validation/7858-settings-workspace.md). |
...
| --- | --- | --- |
| [#7858](https://github.com/atomantic/PortOS/issues/7858) | Settings General and Backup composition, save/error evidence, and responsive validation. | Implemented; synthetic evidence is recorded in [validation/7858-settings-workspace.md](validation/7858-settings-workspace.md). |
...
## Settings implementation follow-up (#7858)

The implementation keeps the existing Settings shell, section navigator, routes, and feature visibility. It changes content order within the two named tabs:

| Surface | Before | After | Evidence boundary |
| --- | --- | --- | --- |
| General | Timezone, location, and theme were three equal stacked cards. | Timezone and location are adjacent at usable container widths and stack below that threshold; theme is a clearly labeled secondary preference. Validation and request failures stay beside the affected save-backed group. | Rendered interaction tests observe the two independent save baselines, dirty guards, failure paths, and the responsive class contract. Exact geometry still needs a browser pass. |
| Backup | Health, configuration, exclusions, snapshots, and restore lived in one long form with the action bar at the end. | Health and saved schedule lead; destination/schedule editing and the saved-state Save/Run actions come before detail; exclusions and snapshot history are named disclosures. The restore dry-run/confirmation remains inside snapshot history. | Rendered interaction tests observe saved-state Run Now gating, degraded status, restore safety, resolved schedules, and the collapsed/expanded snapshot disclosure. Exact sticky-bar geometry still needs a browser pass. |

The [synthetic validation record](validation/7858-settings-workspace.md) separates observed rendered-test facts from hypotheses that require live browser geometry or a full accessibility audit. It uses only invented settings, paths, backup statuses, and snapshot identifiers.
