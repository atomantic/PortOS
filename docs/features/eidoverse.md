...
vendored into PortOS and is not a git submodule. Enabling it from **Settings →
Features** is the explicit consent boundary that downloads and installs the
runtime.
...
at its durable world store. PortOS does not copy, rewrite, or relicense either
project's source.
...
links remain visible on the normal managed-app screen. Bun-managed app updates
use Bun's frozen lockfile rather than npm.
