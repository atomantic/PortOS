...
2. **Analyze without tools or private context.** `runUntrustedContentAnalysis` uses an API text completion, offers no tools or agent harness, and disables provider fallback. Private message sources require a loopback endpoint. Raw messages are not combined with digital-twin identity documents. External text is framed as evidence; framing itself is not an injection detector.
   ...
   **Which decisions are routed.** The issue-watcher reply gate (`reply` / `none`), message triage (action and priority, as two independent decisions over one premise — one abstention retires the whole message), the forge-maintenance disposition, and Stacker News classification/risk. Stacker News is an **escalation-only** gate: after phase-1 screening, jev may settle an item only when classification is `escalate` or risk is `high`; `allowed` / `low` remain shadow observations and the text and vision models still run, so their prose and `disallowedThemes` scan cannot be bypassed. The `allowed` classification option carries a `1.0` floor to encode that it is never acted on. A jev `reply` verdict still wakes the chat model to write the body; the gate only decides whether waking it is worth it.
