...
| **Code Reviewers** | `/settings/code-reviewers` | Configurable multi-reviewer chain (Copilot, Claude, Antigravity, Codex, Grok, Cursor, OpenCode, Kimi, LM Studio, Ollama, MTPLX) with stop conditions, max rounds, per-reviewer model/effort pins, dispute workflows, and the goal-fidelity gate that re-reads a finished CoS run's diff against the task's stated objective (`ship` / `fix-first` / `rethink`) and can, on a finding, file an issue on the project's own tracker (GitHub / GitLab / JIRA, deduplicated by a key carried in the issue body) and/or queue an agent to reconcile it. That agent checks the finding first: when the objective turns out to have been delivered and the reviewer misjudged it, it reports which context the reviewer was missing instead of shipping a change, and PortOS queues a calibration task against its own repo to close that blind spot — deduped on the gap, so many false positives from one cause fold into one fix. | — |
...
| **Voice Mode** | `/settings/voice` | Hands-free voice assistant with continuous VAD (AudioWorklet), speech-to-text (Whisper local or Web Speech), local LLM orchestration, text-to-speech (Piper or Qwen3-TTS), barge-in support, and `ui_navigate` tool execution. | [Voice Mode](./voice.md) |
