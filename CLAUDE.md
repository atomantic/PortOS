...
**Safety-class rules stay in THIS file, even when they're subtree-specific.** Nested `CLAUDE.md` files load on demand in Claude Code, but `getClaudeMdContext()` in `server/services/agentPromptBuilder.js` splices only `~/.claude/CLAUDE.md` and the **workspace-root** `CLAUDE.md` into the prompts PortOS builds for its own API-provider agents — it never walks nested ones. So a rule guarding data loss, a destructive action, privacy, or spend must live here regardless of which subtree it governs (that is why the backup-exclude anchoring rule above sits in root rather than `server/CLAUDE.md`). Relocating a *convention* to a nested file is fine; relocating a *safety rule* silently removes it from an entire class of agent.
