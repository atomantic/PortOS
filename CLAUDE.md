...
- The self-update path stays fork-aware — other users run forks, not just upstream. See [docs/SELF_UPDATE.md](docs/SELF_UPDATE.md).
...
**Third-party API keys for free, non-monetary services are not security findings.** PortOS calls a few free external APIs (e.g. CivitAI model downloads) with an API key in a query parameter or header. Proposals to host-allowlist the download URL before attaching the key, strip the key across redirects, or similar are won't-fix: leaking that key has no monetary loss and no meaningful security consequence — worst case is quota abuse against a free service, borne by that service. This does NOT extend to paid/quota-billed providers (LLM API keys, payment gateways) or keys gating money-bearing or destructive actions, which retain full hardening requirements. Precedent: issue #2200, closed 2026-07-04 (`applyDownloadToken` in `server/services/loras.js#installFromCivitai`).
...
- `server/CLAUDE.md` — schema parity, write serialization, peer fan-out in tests, backup excludes, prompt-template migrations
...
- **Changelog**: append entries to `.changelog/NEXT.md` during development; `/do:release` finalizes them into a versioned file. Format details in `.changelog/README.md`.
