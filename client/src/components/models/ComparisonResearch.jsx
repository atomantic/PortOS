...
export default function ComparisonResearch() {
  ...
  return <section className="bg-port-card border border-port-border rounded-2xl p-5 space-y-4" aria-label="Refresh model research">
    ...
    <div className="flex flex-wrap gap-3 items-end">
      <div className="text-sm min-w-0 max-w-full">{/* Compose is off: the save gate below validates the model against a STORED record's list, which a composed route has none of. */}
      <ProviderModelSelector label="Research provider" compose={false} providers={providers} selectedProviderId={draft.providerId} disabled={busy || !saved} onProviderChange={providerId => setDraft({ providerId, model: '' })} emptyProviderOption="Choose provider" /></div>
