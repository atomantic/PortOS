...
export default function LoomShotPlanner({ loom, episode, route, guidance, disabled, onLoomUpdate }) {
  ...
  return <section className="rounded border border-port-border p-3 space-y-3" aria-label="Timed shot planning">
    ...
    <div className="flex flex-wrap gap-2">
      ...
      <button type="button" disabled={busy || !episode.nodes.length} onClick={() => run(true)} className="min-h-11 rounded bg-port-accent px-3 text-sm text-white disabled:opacity-50">Run shot autopilot</button>
    </div>
    ...
    {plan ? <div className="space-y-2">
      ...
      {!plan.loom ? <button type="button" disabled={busy} onClick={apply} className="min-h-11 rounded bg-port-accent px-3 text-sm text-white">Apply reviewed shots</button> : null}
