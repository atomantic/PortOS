...
describe('LoomPlayPanel', () => {
  ...
  // #8112: without this, the server-side hosted session keeps resolving
  // audience turns against episode 1's graph after the host advances — every
  // turn throws "Scene not found" and silently falls back to canned
  // narration while the UI still claims the session is connected.
  it('re-binds the hosted session before syncing playback when advancing episodes during a hosted session', async () => {
    const user = userEvent.setup();
    const first = {
      id: 'ep-1', number: 1, title: 'One', startNodeId: 'end-1',
      nodes: [{ id: 'end-1', title: 'First ending', prose: 'Episode one ends.', isEnding: true, transitions: [] }],
    ...
    await waitFor(() => expect(hostedSocket.emit).toHaveBeenCalledWith(
      'hosted:episode:switch', { episodeId: 'ep-2' },
    ));

    const calls = hostedSocket.emit.mock.calls;
    const switchIndex = calls.findIndex(([event]) => event === 'hosted:episode:switch');
    const playbackIndex = calls.findIndex(
      ([event, payload]) => event === 'hosted:playback:update' && payload?.nodeId === 'start-2',
    );
    expect(switchIndex).toBeGreaterThanOrEqual(0);
    expect(playbackIndex).toBeGreaterThan(switchIndex);
  });

  it('clears the hosted session and toasts naming the episode when the switch target fails preflight', async () => {
    const user = userEvent.setup();
    const first = {
      id: 'ep-1', number: 1, title: 'One', startNodeId: 'end-1',
      nodes: [{ id: 'end-1', title: 'First ending', prose: 'Episode one ends.', isEnding: true, transitions: [] }],
    ...
    const hostedSocket = io.mock.results.at(-1).value;
    const endedHandler = hostedSocket.on.mock.calls.find(([event]) => event === 'hosted:session:ended')?.[1];
    act(() => { endedHandler({ sessionId: 'host-session', reason: 'episode_not_ready' }); });

    expect(await screen.findByRole('button', { name: 'Host (QR)' })).toBeInTheDocument();
