...
// fetch should render — the polling/retry mechanics belong to useAutoRefetch
...
const { mockUseAutoRefetch, mockGetCosUpcomingTasks } = vi.hoisted(() => ({
  mockUseAutoRefetch: vi.fn(),
...
describe('UpcomingTasksWidget', () => {
  it('renders nothing during the initial load', () => {
    mockUseAutoRefetch.mockReturnValue({ data: null, loading: true, error: null });
    const { container } = renderWidget();
    expect(container).toBeEmptyDOMElement();
  });

  it('renders nothing when there is genuinely no upcoming work and no error', () => {
    mockUseAutoRefetch.mockReturnValue({ data: [], loading: false, error: null });
    const { container } = renderWidget();
    expect(container).toBeEmptyDOMElement();
  });

  it('shows an unavailable state instead of silently hiding when the first-ever fetch fails (#7527)', () => {
    mockUseAutoRefetch.mockReturnValue({ data: null, loading: false, error: new Error('boom') });
    renderWidget();
