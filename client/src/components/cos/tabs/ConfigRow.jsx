...
export default function ConfigRow({
  ...
  const [numberDraft, setNumberDraft] = useState(String(inputValue));

  useEffect(() => {
    if (!numberFocusedRef.current) setNumberDraft(String(inputValue));
  }, [inputValue]);
  ...
  const commitNumber = (event) => {
    ...
    if (
      ...
      setNumberDraft(String(inputValue));
      return;
    }
      ...
      {isCheckbox ? (
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            step={step}
            value={numberDraft}
            onFocus={() => { numberFocusedRef.current = true; }}
