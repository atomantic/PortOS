    ...
    <div ref={node => { drag.setNodeRef(node); drop.setNodeRef(node); }}
      ...
      {children(
        <button type="button" ref={node => { drag.setActivatorNodeRef(node); if (node) handles.current.set(id, node); else handles.current.delete(id); }}
          ...
          aria-label={`Drag ${data.label}`} className="touch-none min-h-11 sm:min-h-8 min-w-11 sm:min-w-8 flex items-center justify-center text-gray-400 hover:text-white disabled:opacity-40">
...
export default function ReviewerGroupsEditor({ groups, onGroupsChange, reviewerHealth = EMPTY_HEALTH, onChange, disabled = false, ...pickerProps }) {
  ...
  const moveMember = (sourceId, token, destinationId, beforeToken) => {
    ...
    changeGroups(next, `${token} moved to ${tierName(groups.indexOf(destination))}. Configured providers and local models run first.`, memberId(destinationId, token));
  };
  ...
  return (
    <div className="space-y-3 min-w-0">
      <DndContext sensors={sensors} collisionDetection={collisionDetection} onDragEnd={drop}
        accessibility={{
          screenReaderInstructions: { draggable: 'Press Space to pick up. Use arrow keys to move between reviewers or tiers. Press Space to drop, or Escape to cancel. Configured providers and local models always run first.' },
          announcements: {
            ...
            onDragEnd: ({ active, over }) => over ? `Dropped ${active.data.current.label} at ${over.data.current.label}. Configured providers and local models run first.` : 'Move cancelled.',
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button type="button" disabled={disabled} className="min-h-11 sm:min-h-8 px-3 text-sm text-port-accent border border-port-border rounded disabled:opacity-40"
