...
export default function VideoShotEditor({ project, scene, onChange }) {
  ...
  const dirty = prompt !== scene.prompt || sourceImageFile !== (scene.sourceImageFile || null) || continuation !== Boolean(scene.useContinuationFromPrior);
  const save = async () => {
    if (saving || active || !dirty || !prompt.trim()) return;
    setSaving(true);
    const saved = await updateCreativeDirectorScene(project.id, scene.sceneId, {
      prompt, sourceImageFile, useContinuationFromPrior: continuation, expectedWorkRevision: scene.workRevision || 0,
