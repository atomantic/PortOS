...
Work in this app's actual configured checkout, not a fresh worktree that hides its dirty state. Read its repository instructions and discover its path, origin, default branch and branch protections first. Respect forks: origin is the target, never substitute upstream. Keep scope to this selected app's checkout; inspect other managed or companion repositories only when this app's own repository configuration directly depends on them. Inspect linked worktrees and active agents only to protect ownership, and do not modify or remove unrelated worktrees.
...
Fetch origin and compare each pending change against current remote history. Decide whether it is redundant because the current origin already contains an equivalent fix, or whether this app needs a PR for a unique change. Preserve the newer implementation while retaining every unique intended change. Finish only relevant work, run appropriate checks, review the diff for secrets and quality, and commit only relevant files. Push completed work normally; if branch protection requires a PR, create it and satisfy required CI/review gates before merging. Do not bypass protections.
