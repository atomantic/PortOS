...
export default function AppQualityHistory({ appId, categories = [] }) {
  ...
  return (
    <section aria-label="Quality over time" className="bg-port-card border border-port-border rounded-lg p-4 space-y-3 min-w-0">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <label htmlFor="quality-period" className="sr-only">Period</label>
          <select id="quality-period" className="bg-port-bg border border-port-border rounded p-1" value={days} onChange={e => change('qualityDays', e.target.value)}>
            <option value="30">30 days</option><option value="90">90 days</option><option value="365">1 year</option>
          </select>
          <label htmlFor="quality-history-category" className="sr-only">Category</label>
          <select id="quality-history-category" className="bg-port-bg border border-port-border rounded p-1 max-w-[12rem]" value={category} onChange={e => change('qualityCategory', e.target.value)}>
            <option value="overall">Overall</option>{categories.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
          </select>
          <button type="button" aria-label="Refresh history" title="Refresh history" className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center text-gray-400 hover:text-port-accent" onClick={() => setRefresh(n => n + 1)}>
      ...
      {state.loading ? <p role="status">Loading quality history…</p> : state.error ? <p role="alert">Quality history could not be loaded. Use Refresh history to retry.</p> : !measured.length ? <p className="text-sm text-gray-400">No scored assessments in this period. Run an audit to start the history.</p> : <>
        ...
        <details><summary className="cursor-pointer text-xs text-port-accent">History values and coverage</summary>
