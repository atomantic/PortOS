/**
 * Generic "Queue agent to investigate" action (#5981) for any UI-detected
 * failure that should hand a reproducible task to the CoS queue.
 *
 * The caller builds the task (`{ description, prompt }`, e.g. via
 * `buildInstallFailureTask` or `buildSelectorTestFailureTask`) — this
 * component owns everything downstream: posting it with the durable
 * `isInvestigation` marker, handling the store's 409 `DUPLICATE_TASK` as an
 * "already queued" state rather than an error, and the three button states.
 * `QueueInstallInvestigationButton` is a thin wrapper of this for its
 * existing installer-failure call sites.
 */
...
export default function QueueInvestigationButton({ task, className = '' }) {
  ...
  const [queueTask, queueing] = useAsyncAction(async () => {
    // The description is deterministic per failure, so retrying the same
    // failure and clicking again hits the store's duplicate guard (409
    // DUPLICATE_TASK). A task already exists — not a failure to report.
    const duplicateMessage = await addCosTask({ ...task, ...INVESTIGATION_MARKER }, { silent: true })
      .then(() => null)
      .catch((err) => {
    ...
    <button
      ...
      title={queueResult === 'queued'
        ...
        : queueResult === 'duplicate'
          ...
          : 'Queue a PortOS agent task to investigate this failure'}
