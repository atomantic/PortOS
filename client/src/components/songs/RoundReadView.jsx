  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {isNonBlankStr(song.artist) && metaBadge('Artist', song.artist)}
        {isNonBlankStr(song.key) && metaBadge('Key', song.key)}
                ...
                {layers.map((l) => (
                  <li key={l.id} className="bg-port-card border border-port-border rounded-lg p-3">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      {isNonBlankStr(l.part) && <span className="text-xs text-gray-500">{l.part}</span>}
                    </div>
                    {isNonBlankStr(l.notes) && <p className="mt-1 text-xs text-gray-400 whitespace-pre-wrap leading-relaxed">{l.notes}</p>}
                  </li>
                ))}
              </ul>
            </section>
