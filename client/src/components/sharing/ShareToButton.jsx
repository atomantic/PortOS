...
export default function ShareToButton({ kind, ids, items, label = 'Share', compact = false, className = '' }) {
  ...
  // card). `usePopoverPosition` owns the fixed placement: it anchors to the
  // trigger's viewport rect, clamps into the viewport, flips above the trigger
  // when there is no room below, and re-flows on capture-phase scroll. The
  // bucket list changes the menu's height as it loads, so it re-measures via
  // contentDeps.
  const { triggerRef, popoverRef, style: menuStyle } = usePopoverPosition({
    ...
    gap: 4,
    position: 'below',
    contentDeps: [loading, buckets.length, subscriptions.length]
  });
