...
| `useVisionModelIds` | `{ idsByBackend, loaded }` — vision-capable model ids the server reports, keyed by local backend (`{ ollama: Set, lmstudio: Set }` from Ollama `/api/show` + LM Studio `type:'vlm'`), `null` until fetched; `loaded` flips once the fetch settles. Drops `backend:'cli'` rows (a blanket per-provider claim, not a per-model fact). | Any VISION model picker — pass `idsByBackend` to `visionLocalModelFilter` / `assignmentModelOptions` so a new multimodal family (`gemma4`) isn't hidden by the stale client id regex; gate any "none installed" messaging on `loaded` and on the provider being a local backend. |
