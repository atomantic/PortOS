...
| `useAutoRefetch` | Poll-based refetch on an interval; `{ pollOnly: true }` drops the data/loading bookkeeping and returns `{ refetch }` only. | Data needs periodic refresh (no socket / SSE available). Use `pollOnly` when the fetchFn already updates state via setStates inside its body. |
