...
| `useKeyboardShortcuts` | `useKeyboardShortcuts(active, bindings)` — fire a `{ key: handler }` map while `active`; ignores events from editable fields (`isEditableTarget` also exported) and ⌘/Ctrl/Alt chords; a falsy handler disables that key. | Single-surface action shortcuts (editorial comment card prev/next + accept/dismiss/generate, #1603). Not for held-key/game input (that's `useKeyboardControls`). |
