...
| `useVisionModelIds` | `Set` of model ids the server reports as vision-capable from each backend's own capability metadata (Ollama `/api/show`, LM Studio `type:'vlm'`); `null` until fetched. | Any VISION model picker — pass to `visionLocalModelFilter` / `assignmentModelOptions` so a new multimodal family (`gemma4`) isn't hidden by the stale client id regex. |
