...
| `useProcessLogs` | Live PM2 log tail for one process over the shared socket: owns the `logs:subscribe`/`line`/`subscribed`/`error` lifecycle, per-`processName` frame filtering, and a 1000-line ring buffer. Pass `appId` so an app with its own PM2_HOME streams from the right home. Returns `{ logs, subscribed }`. | Any surface showing a PM2 process's live output (Processes tab, desktop-app launch progress). **One stream per socket** — never mount two at once. |
