...
describe('useBeeperOutbox', () => {
  ...
  it('keeps delayed requests scoped to their original conversation', async () => {
    let finishSend;
    let finishList;
    const onSent = vi.fn();
    sendOutboxEntry.mockImplementationOnce(() => new Promise((resolve) => { finishSend = resolve; }));
    const { result, rerender } = renderHook(({ id }) => useBeeperOutbox(id, { onSent }), {
      initialProps: { id: CONVERSATION_ID },
    });
    await act(async () => {});
    let sending;
    await act(async () => { sending = result.current.submit('hello there'); });
    listOutboxEntries.mockImplementationOnce(() => new Promise((resolve) => { finishList = resolve; }));
    let refresh;
    act(() => { refresh = result.current.refresh(); });
    rerender({ id: 'other-conversation' });
    await act(async () => {});
    await act(async () => {
      finishSend({ ...ENTRY, state: 'awaiting-confirmation' });
    ...
    expect(onSent).toHaveBeenCalledWith(CONVERSATION_ID, ENTRY.body);
  });
