 ...
 * @param {(conversationId: string, body: string) => void} [options.onSent]
 *   Called after a composer send is accepted, with its original conversation
 *   and text so navigation or a newer edit cannot clear an unrelated draft. A send
...
export default function useBeeperOutbox(conversationId, { onSent } = {}) {
  ...
  const scopeRef = useRef({ conversationId, refreshGeneration: 0 });
  if (scopeRef.current.conversationId !== conversationId) {
    scopeRef.current = { conversationId, refreshGeneration: 0 };
  }
  ...
  const refresh = useCallback(async () => {
    ...
    if (!mountedRef.current || scopeRef.current !== scope || generation !== scope.refreshGeneration) return;
  ...
  useBeeperRealtime({
    onInvalidate: (frame) => {
      if (!['message.upserted', 'outbox.updated'].includes(frame?.kind) || !awaitingRef.current) return;
      refresh();
    },
  });
  ...
  const dispatch = useCallback(async (entry, confirmFirstContact, clearsDraft = false) => {
    ...
    if (!err && clearsDraft) onSentRef.current?.(conversationId, entry.body);
    if (scopeRef.current !== scope) return !err;
    setSending(false);
  ...
  }, [conversationId, dispatch, sending, scope]);
