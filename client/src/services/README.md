...
| `apiModelComparison.js` | PortOS benchmark observations, provider model discovery, and explicit benchmark runs. |
...
| `apiCodeAnimation.js` | Code Animation: host-contract options, LLM-written briefs from a universe, universe-styled LLM prompt building, a persistent generation gallery, and provider jobs (start + poll for the HTML). |
