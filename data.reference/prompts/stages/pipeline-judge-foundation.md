...
3. **structure** *(weight 20%)* — Does the opening plan establish the protagonist's immediate personal want, ordinary relationships, pressure on an existing belief, a costly choice, and visible aftermath? A mission briefing, banter, inventory discovery, and cliffhanger are not a complete first-issue arc. Does each principal cause distinct turns rather than merely accompany the plot? Track relationships through setup, rupture, attempted repair, and earned change. Is the arc outline complete (logline, summary, protagonist arc, per-volume loglines + ending hooks)? Is foreshadowing balanced (setups that will pay off, not everything front-loaded or nothing planted)? Do the volume threads nest coherently toward the finale? An opening with no personal causal arc cannot score above 5, even if all outline fields are filled.
