...
- **Shared `<Modal>` component + `getRenderConfigForItem` extraction** — Two more "Civitai LoRA / Z-Image-Turbo follow-ups" off the backlog. (a) New `client/src/components/ui/Modal.jsx` (~100 LOC) owns the `fixed inset-0 z-50 bg-black/70 ...` backdrop, Esc handler, target-checked click-outside, ARIA + portal + size + align flags. Converted 9 call sites: `Flux2InstallModal`, `EditAppModal`, `MemoryEditModal`, `ResumeAgentModal`, `LayoutEditor`, `KeyboardHelp`, `RapidReaderModal`, `DeployPanel`, and `CivitaiAuthModal` in `Loras.jsx`; net dedup is ~40 LOC after per-site config props and unblocks future modals from owning chrome boilerplate. `MediaLightbox` deliberately stays standalone — its prev/next viewport-edge buttons + layered Esc cascade fight Modal's panel-container wrapper; comment in the file documents why. (b) `getRenderConfigForItem(item)` lives in `client/src/components/media/normalize.js`, surfacing sidecar fields (`cfgScale`/`cfg_scale`, `loraFilenames`/`lora_filenames`, `loraScales`/`lora_scales`, `guidanceScale`/`guidance_scale`/`guidance`, `disableAudio`/`disable_audio`, etc.) so `PromptRefineModal#getRenderConfig` stopped reaching into `item.raw?.*` for snake/camel reconciliation. 11 new unit tests in `normalize.test.js` cover both naming conventions, the deliberate-`0` guidanceScale carve-out, and the camelCase-wins drift safety. Server's `vitest.config.js` now picks up `../client/src/**/*.test.js` so client-side pure helpers stay covered without bootstrapping a separate runner.
