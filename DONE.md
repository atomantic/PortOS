...
- **Civitai / Z-Image follow-ups — six post-merge cleanups** — Six items off the "Civitai LoRA / Z-Image-Turbo follow-ups" backlog in one pass.
  ...
  - **Generic `deepMerge` utility** — New `server/lib/objects.js#deepMerge` adopts the cleanest existing implementation (from `voice/config.js`). Three call sites collapsed: `services/voice/config.js`, `services/meatspacePost.js`, and `routes/loras.js#POST /auth/civitai`. Documents array-replacement + null-as-value semantics; 9 unit tests cover override, null, missing-base, mutation guard, and the patch-not-an-object early return.
  - **`assertSafeFilename` helper** — New shared `server/lib/fileUtils.js#assertSafeFilename(filename, { extensions, subject })` consolidates `services/loras.js#assertSafeLoraFilename` (`.safetensors`) and `services/imageGen/local.js#assertGalleryFilename` (`.png`). Rejects path separators, null bytes, exact `.`/`..`, and non-allowlisted extensions; case-insensitive extension match; throws a 400 ServerError with code `VALIDATION_ERROR`. 11 unit tests in `fileUtils.test.js`.
  - **`mediaModels.js` drift warning** — `loadMediaModels()` now logs a loud `⚠️ media-models drift: built-in "<id>" was shipped but is missing from <kind>[] — restore it manually or delete _shippedDefaults.<kind> to re-bootstrap` warning at boot for any id present in BOTH `_shippedDefaults.<kind>.list` AND `DEFAULT_REGISTRY.<kind>` but missing from the user's live list. Catches the 2026-05-09 silent-drift class. No auto-recover — surfaces the bug, lets a human decide. Covers `image` plus `video.macos` and `video.windows`. 4 new tests.
  - **CLI `defaultModel` for all CLI providers** — `services/runner.js#buildCliArgs` now appends `--model <id>` to claude-code (after `-p -`; claude-code parses flags positionally) and `-m <id>` to gemini-cli, gated on the provider's stored `args` not already containing `--model`/`-m` (both space- and `=`-joined forms). Codex behavior unchanged. Stage runner / message evaluator / media prompt refiner per-call model overrides now actually take effect for all three CLIs instead of silently no-op'ing for claude-code and gemini-cli. 8 new tests + 2 regression tests; surfaced from Copilot review on PR #217.
